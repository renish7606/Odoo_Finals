<<<<<<<< HEAD:frontend/dist/assets/index-B0A2WwhW.js
...
========
(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))r(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&r(i)}).observe(document,{childList:!0,subtree:!0});function n(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function r(s){if(s.ep)return;s.ep=!0;const a=n(s);fetch(s.href,a)}})();const G={},et=(G==null?void 0:G.VITE_API_BASE_URL)||"/api/v1";class H extends Error{constructor(e,n,r){super(e),this.name="ApiError",this.status=n,this.data=r}}const m={getToken(){return localStorage.getItem("dealflow_token")},setToken(t){t?localStorage.setItem("dealflow_token",t):localStorage.removeItem("dealflow_token")},getHeaders(t={}){const e={"Content-Type":"application/json",...t},n=this.getToken();return n&&(e.Authorization=`Bearer ${n}`),e},async request(t,e={}){const n=`${et}${t}`,r=this.getHeaders(e.headers),s={...e,headers:r};s.body&&typeof s.body=="object"&&!(s.body instanceof FormData)&&(s.body=JSON.stringify(s.body));try{const a=await fetch(n,s);if(a.status===401&&!t.includes("/auth/login"))throw this.setToken(null),localStorage.removeItem("dealflow_user"),window.location.hash="#/login",new H("Session expired. Please sign in again.",401,null);if(a.status===204)return null;const i=a.headers.get("content-type")||"";let l=null;if(i.includes("application/json")?l=await a.json():l=await a.text(),!a.ok){const o=(l==null?void 0:l.detail)||(l==null?void 0:l.message)||`Request failed with status ${a.status}`;throw new H(o,a.status,l)}return l}catch(a){throw a instanceof H?a:new H(a.message||"Network connection failed",0,null)}},get(t,e){let n=t;if(e){const r=new URLSearchParams(e).toString();r&&(n+=`?${r}`)}return this.request(n,{method:"GET"})},post(t,e){return this.request(t,{method:"POST",body:e})},put(t,e){return this.request(t,{method:"PUT",body:e})},patch(t,e){return this.request(t,{method:"PATCH",body:e})},delete(t){return this.request(t,{method:"DELETE"})},async downloadFile(t,e){const n=`${et}${t}`,r=this.getToken(),s=r?{Authorization:`Bearer ${r}`}:{},a=await fetch(n,{headers:s});if(!a.ok)throw new Error(`Download failed with status ${a.status}`);const i=await a.blob(),l=window.URL.createObjectURL(i),o=document.createElement("a");o.href=l,o.download=e||"document.pdf",document.body.appendChild(o),o.click(),o.remove(),window.URL.revokeObjectURL(l)}},_={getUser(){try{const t=localStorage.getItem("dealflow_user");return t?JSON.parse(t):null}catch{return null}},setUser(t){t?localStorage.setItem("dealflow_user",JSON.stringify(t)):localStorage.removeItem("dealflow_user")},isAuthenticated(){return!!m.getToken()},async login(t,e,n){const r=await m.post("/auth/login",{email:t,password:e});if(r.access_token){m.setToken(r.access_token);try{const s=await m.get("/auth/me");return s.selected_role=n||s.role,this.setUser(s),{success:!0,user:s}}catch{const s={email:t,full_name:t.split("@")[0],role:n||"SalesRep",selected_role:n||"SalesRep"};return this.setUser(s),{success:!0,user:s}}}throw new Error("Authentication failed: No access token received")},async signup({full_name:t,email:e,password:n,role:r}){return await m.post("/auth/signup",{full_name:t,email:e,password:n,role:r})},logout(){m.setToken(null),this.setUser(null),window.location.hash="#/login"}};function h(t="quotations"){const e=_.getUser()||{full_name:"User",role:"SalesRep"},n=e.selected_role||e.role||"",r=[{key:"dashboard",label:"Dashboard",icon:"dashboard",href:"#/dashboard"},{key:"quotations",label:"Quotations",icon:"request_quote",href:"#/quotations"},{key:"approvals",label:"Approvals",icon:"verified",href:"#/approvals"},{key:"deal-health",label:"Deal Health",icon:"crisis_alert",href:"#/deal-health"},{key:"fulfillment",label:"Fulfillment",icon:"assignment_turned_in",href:"#/fulfillment"},{key:"invoices",label:"Invoices",icon:"receipt_long",href:"#/invoices"},{key:"products",label:"Products",icon:"inventory_2",href:"#/products"},{key:"pricing",label:"Pricing",icon:"sell",href:"#/pricing"},{key:"subscriptions",label:"Subscriptions",icon:"sync",href:"#/subscriptions"},{key:"reports",label:"Reports",icon:"bar_chart",href:"#/reports"},{key:"messages",label:"Messages",icon:"chat_bubble",href:"#/messages"},{key:"profile",label:"Profile",icon:"account_circle",href:"#/profile"}],s={SalesRep:["dashboard","quotations","customers","products"],SalesManager:["dashboard","quotations","approvals","pricing","reports"],FinanceOps:["dashboard","approvals","fulfillment","invoices","subscriptions"],Admin:["dashboard","quotations","approvals","deal-health","fulfillment","invoices","customers","products","pricing","subscriptions","reports"],Customer:["quotations","messages","profile"]},a=s[n]||s.SalesRep;return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
<<<<<<<< HEAD:frontend/dist/assets/index-B0A2WwhW.js
        ...
        <a href="${o.href}" class="nav-item ${l}" data-route="${o.key}">
          ...
          <span>${o.label}</span>
========
          ${r.filter(o=>a.includes(o.key)).map(o=>{const d=t===o.key||t==="quotation-detail"&&o.key==="quotations"?"nav-item-active":"nav-item-inactive",u=n==="Customer"&&o.key==="quotations"?"My Quotations":o.label;return`
...
========
  `}function w(){const t=document.getElementById("btn-logout");t&&t.addEventListener("click",()=>{confirm("Are you sure you want to sign out?")&&_.logout()})}function ft(){return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
  `}function xt(){const t=document.getElementById("login-form"),e=document.getElementById("login-error"),n=document.getElementById("btn-submit-login"),r=document.getElementById("toggle-pw"),s=document.getElementById("login-password"),a=document.getElementById("pw-icon");r&&s&&a&&r.addEventListener("click",()=>{s.type==="password"?(s.type="text",a.textContent="visibility_off"):(s.type="password",a.textContent="visibility")}),t&&t.addEventListener("submit",async i=>{i.preventDefault(),e.classList.add("hidden"),e.textContent="",n.disabled=!0,n.innerHTML='<span class="loading-spinner"></span> Authenticating...';const l=document.getElementById("login-email").value.trim(),o=s.value.trim();try{await _.login(l,o),window.location.hash="#/quotations"}catch(c){e.textContent=c.message||"Login failed. Check your credentials.",e.classList.remove("hidden")}finally{n.disabled=!1,n.innerHTML='<span><span>Sign In to DealFlow360</span><span class="material-symbols-outlined text-base">arrow_forward</span></span>'}})}function vt(){return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
    <div class="login-wrapper flex items-center justify-center min-h-screen w-full p-4">
      <!-- Ambient clay orbs -->
      <div class="ambient-orb orb-1"></div>
...
========
  `}function bt(){const t=document.getElementById("signup-form"),e=document.getElementById("signup-error"),n=document.getElementById("signup-success"),r=document.getElementById("btn-submit-signup"),s=document.getElementById("signup-toggle-pw"),a=document.getElementById("signup-password"),i=document.getElementById("signup-pw-icon");s&&a&&i&&s.addEventListener("click",()=>{a.type==="password"?(a.type="text",i.textContent="visibility_off"):(a.type="password",i.textContent="visibility")}),t&&t.addEventListener("submit",async l=>{l.preventDefault(),e.classList.add("hidden"),e.textContent="",n.classList.add("hidden"),n.textContent="",r.disabled=!0,r.innerHTML='<span class="loading-spinner"></span> Creating Account...';const o=document.getElementById("signup-name").value.trim(),c=document.getElementById("signup-email").value.trim(),d=a.value,u=document.getElementById("signup-confirm-password").value;if(d!==u){e.textContent="Passwords do not match.",e.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<span>Create Account</span><span class="material-symbols-outlined text-base">person_add</span>';return}if(d.length<8){e.textContent="Password must be at least 8 characters.",e.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<span>Create Account</span><span class="material-symbols-outlined text-base">person_add</span>';return}try{await _.signup({full_name:o,email:c,password:d,role:"SalesRep"}),n.textContent="Account created successfully! Redirecting to login...",n.classList.remove("hidden"),t.reset(),setTimeout(()=>{window.location.hash="#/login"},1500)}catch(p){e.textContent=p.message||"Signup failed. Please try again.",e.classList.remove("hidden")}finally{r.disabled=!1,r.innerHTML='<span>Create Account</span><span class="material-symbols-outlined text-base">person_add</span>'}})}class gt{constructor(){this.deals=[],this.listeners=[],this.loaded=!1}subscribe(e){return this.listeners.push(e),()=>{this.listeners=this.listeners.filter(n=>n!==e)}}notify(){this.listeners.forEach(e=>e(this.deals)),this.saveToStorage()}saveToStorage(){localStorage.setItem("dealHealthState",JSON.stringify(this.deals))}loadFromStorage(){const e=localStorage.getItem("dealHealthState");if(e)try{return this.deals=JSON.parse(e),this.loaded=!0,!0}catch(n){console.error("Failed to parse deal health state",n)}return!1}async fetchHealthData(){this.loadFromStorage(),this.notify();try{const[e,n,r,s]=await Promise.all([m.get("/deal_health/stalled").catch(()=>[]),m.get("/deal_health/anomalies").catch(()=>[]),m.get("/deal_health/slippage").catch(()=>[]),m.get("/quotations").catch(()=>[])]),a={};s.forEach(o=>a[o.id]=o);const i={},l=(o,c,d)=>{if(!i[o]){const u=a[o];if(!u)return;i[o]={id:u.id,dealRef:u.deal_reference,dealName:`Quote ${u.deal_reference}`,customerName:u.customer_name||"Unknown",repName:u.rep_name||"Unknown",issues:[],issueDetails:{},flaggedDate:new Date().toISOString().split("T")[0],actionStatus:"Action Required"}}i[o].issues.includes(c)||(i[o].issues.push(c),i[o].issueDetails[c]=d)};e.forEach(o=>l(o.quotation_id,"stalled",{daysInactive:o.days_inactive})),n.forEach(o=>l(o.quotation_id,"discount_anomaly",{discountGiven:o.discount_given,repAvg:o.rep_average_discount})),r.forEach(o=>l(o.quotation_id,"delivery_slippage",{daysDelayed:o.days_delayed})),this.deals=Object.values(i),this.loaded=!0,this.notify()}catch(e){console.error("Failed to fetch deal health data",e)}}getStalledDeals(){return this.deals.filter(e=>e.issues.includes("stalled"))}getDiscountAnomalies(){return this.deals.filter(e=>e.issues.includes("discount_anomaly"))}getDeliverySlippages(){return this.deals.filter(e=>e.issues.includes("delivery_slippage"))}getAtRiskDeals(){return this.deals.filter(e=>e.issues.length>0)}getAtRiskCount(){return this.getAtRiskDeals().length}getActiveFlagsCount(){return this.deals.reduce((e,n)=>e+n.issues.length,0)}getFlaggedItems(){return this.deals.filter(e=>e.issues.length>0).map(e=>{var r,s,a,i,l,o;const n=[];if(e.issues.includes("stalled")&&n.push(`Idle ${((s=(r=e.issueDetails)==null?void 0:r.stalled)==null?void 0:s.daysInactive)||7}+ days`),e.issues.includes("discount_anomaly")){const c=((i=(a=e.issueDetails)==null?void 0:a.discount_anomaly)==null?void 0:i.discountGiven)||0,d=((o=(l=e.issueDetails)==null?void 0:l.discount_anomaly)==null?void 0:o.repAvg)||0;n.push(`Discount ${c}% vs avg ${d}%`)}return e.issues.includes("delivery_slippage")&&n.push("Fulfillment delayed"),{...e,displayIssue:n.join(" & ")}})}async nudgeRep(e){const n=this.deals.find(r=>r.id===e);if(n)try{await m.post(`/deal_health/${e}/nudge`),n.actionStatus="Nudge sent",this.notify()}catch(r){console.error("Failed to nudge",r)}}async escalateDeal(e){const n=this.deals.find(r=>r.id===e);if(n)try{await m.post(`/deal_health/${e}/escalate`),n.actionStatus="Escalated to Manager",this.notify()}catch(r){console.error("Failed to escalate",r)}}async resolveRisk(e,n=null){const r=this.deals.find(s=>s.id===e);if(r)try{await m.post(`/deal_health/${e}/resolve`,{issue_type:n}),n?r.issues=r.issues.filter(s=>s!==n):r.issues=[],r.issues.length===0&&(r.actionStatus="Resolved"),this.notify()}catch(s){console.error("Failed to resolve",s)}}simulateStalledDeal(e){let n=this.deals.find(r=>r.id===e.id);n?n.issues.includes("stalled")||(n.issues.push("stalled"),n.issueDetails||(n.issueDetails={}),n.issueDetails.stalled={daysInactive:8},n.actionStatus="Action Required",this.notify()):(this.deals.push({id:e.id,dealRef:e.deal_reference,dealName:`Quote ${e.deal_reference}`,customerName:e.customer_name||"Unknown",repName:e.rep_name||"Unknown",issues:["stalled"],issueDetails:{stalled:{daysInactive:8}},flaggedDate:new Date().toISOString().split("T")[0],actionStatus:"Action Required"}),this.notify())}}const L=new gt;function yt(t={}){const{summary:e={},quotations:n=[]}=t,r=e.total_revenue?`₹${Number(e.total_revenue).toLocaleString("en-IN")}`:"₹4,82,05,000",s=e.pending_approvals??1,a=(e.total_quotations||e.draft_count)&&e.total_quotations||5;e.win_rate&&`${e.win_rate}`;const i=L.getAtRiskCount(),l=L.getActiveFlagsCount(),o=L.getStalledDeals().length,c=L.getDeliverySlippages().length,u=(n.length>0?n.slice(0,6):[{id:8492,deal_reference:"DEAL-0005",rep_name:"Marcus Vance",customer_name:"Terra Motors OEM",total_amount:0,status:"Approved",time:"Recent"},{id:8488,deal_reference:"DEAL-0004",rep_name:"Eleanor Vance",customer_name:"Zenith Retail AI",total_amount:0,status:"Pending Approval",time:"Recent"},{id:8475,deal_reference:"DEAL-0003",rep_name:"Marcus Vance",customer_name:"Starlight Dynamics Inc.",total_amount:0,status:"Under Negotiation",time:"Recent"},{id:8461,deal_reference:"DEAL-0001",rep_name:"Local Sales Rep",customer_name:"Bronze Buyer",total_amount:1300,status:"Draft",time:"Recent"},{id:8462,deal_reference:"DEAL-0002",rep_name:"Local Sales Manager",customer_name:"Gold Buyer",total_amount:1500,status:"Confirmed",time:"Recent"}]).map(p=>{let x="badge-primary";const v=(p.status||"").toLowerCase(),$=v.includes("approve")||v.includes("pending")||v.includes("negotiat");v.includes("approve")?x="badge-success":v.includes("negotiat")||v.includes("review")||v.includes("pending")?x="badge-warning":v.includes("fulfill")||v.includes("confirm")?x="badge-info":v.includes("draft")&&(x="badge-neutral");const k=p.rep_name||p.rep&&p.rep.full_name||"Sales Rep",S=k.split(" ").map(N=>N[0]).join("").substring(0,2).toUpperCase()||"SR",y=p.customer_name||p.customer&&p.customer.name||"Client Organization",D=p.deal_reference||(p.id?`DEAL-${String(p.id).padStart(4,"0")}`:"DEAL-0001");return`
      <tr class="table-row hover:bg-surface-container/50 transition-colors activity-row" data-status="${p.status||"Draft"}" data-is-approval="${$?"true":"false"}" data-deal-id="${p.id}">
        <td class="py-3 px-4 text-xs font-mono text-on-surface-variant">${p.time||"Recent"}</td>
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
        <td class="py-3 px-4">
...
========
        ...
        <td class="py-3 px-4">
          <span class="badge ${x}">● ${p.status||"Draft"}</span>
        </td>
        <td class="py-3 px-4 text-right">
          <a href="#/quotations/${p.id}" class="btn btn-secondary text-xs py-1 px-3 btn-review-deal" data-id="${p.id}">Review Deal</a>
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
        </td>
      </tr>
    `}).join("");return`
...
========
  `}async function ht(){try{const[t,e]=await Promise.all([m.get("/dashboard/summary").catch(()=>({})),m.get("/quotations").catch(()=>[]),L.fetchHealthData()]);return{summary:t,quotations:e}}catch{return{summary:{},quotations:[]}}}function wt(){const t=document.getElementById("btn-new-deal-dash");t&&t.addEventListener("click",()=>{window.location.hash="#/quotation-detail"});const e=document.getElementById("btn-quarter-toggle"),n=document.getElementById("quarter-dropdown-menu"),r=document.getElementById("current-quarter-label"),s=document.getElementById("quarter-chevron");e&&n&&(e.addEventListener("click",f=>{f.stopPropagation(),n.classList.contains("show")?(n.classList.remove("show"),s.style.transform="rotate(0deg)"):(n.classList.add("show"),s.style.transform="rotate(180deg)")}),document.querySelectorAll(".quarter-menu-item").forEach(f=>{f.addEventListener("click",()=>{document.querySelectorAll(".quarter-menu-item").forEach(b=>b.classList.remove("selected")),f.classList.add("selected"),r&&(r.textContent=f.getAttribute("data-label")),n.classList.remove("show"),s&&(s.style.transform="rotate(0deg)")})}),document.addEventListener("click",f=>{f.target.closest("#quarter-selector-wrapper")||(n.classList.remove("show"),s&&(s.style.transform="rotate(0deg)"))})),document.getElementById("velocity-chart-card");const a=document.getElementById("velocity-chart-tooltip"),i=document.getElementById("vtt-month"),l=document.getElementById("vtt-actual"),o=document.getElementById("vtt-target"),c=document.getElementById("vtt-variance"),d=document.getElementById("vtt-status"),u=document.querySelectorAll(".velocity-node-group"),p=document.querySelectorAll(".velocity-month-col"),x=document.getElementById("target-plan-line"),v=[{month:"July",actual:122e4,target:11e5,leftPct:18},{month:"August",actual:1685500,target:142e4,leftPct:50},{month:"September (Current)",actual:1915e3,target:17e5,leftPct:82}];function $(f){const b=v[f];if(!b||!a)return;u.forEach((E,C)=>{C===f?(E.classList.add("active"),E.style.opacity="1"):(E.classList.remove("active"),E.style.opacity="0.5")});for(let E=0;E<3;E++){const C=document.getElementById(`vconn-${E}`),F=document.getElementById(`vtgt-${E}`);C&&(C.style.opacity=E===f?"1":"0"),F&&(F.style.opacity=E===f?"1":"0")}const g=b.actual-b.target,B=(g/b.target*100).toFixed(1),P=g>=0?"+":"-",I=g>=0;i&&(i.textContent=b.month),l&&(l.textContent=`₹${Number(b.actual).toLocaleString("en-IN")}`),o&&(o.textContent=`₹${Number(b.target).toLocaleString("en-IN")}`),c&&(c.textContent=`${P}₹${Number(Math.abs(g)).toLocaleString("en-IN")}`,c.style.color=I?"#2e6930":"var(--color-error)"),d&&(d.textContent=`${P}${B}% ${I?"Ahead of Target":"Behind Target"}`,d.className=`velocity-tt-status ${I?"ahead":"behind"}`),f===0?(a.style.left=`${b.leftPct}%`,a.style.top="12px",a.style.transform="translate(-25%, 0)"):f===1?(a.style.left=`${b.leftPct}%`,a.style.top="12px",a.style.transform="translate(-50%, 0)"):(a.style.left=`${b.leftPct}%`,a.style.top="12px",a.style.transform="translate(-75%, 0)"),a.classList.add("visible")}function k(){u.forEach(f=>{f.classList.remove("active"),f.style.opacity="1"});for(let f=0;f<3;f++){const b=document.getElementById(`vconn-${f}`),g=document.getElementById(`vtgt-${f}`);b&&(b.style.opacity="0"),g&&(g.style.opacity="0")}a&&a.classList.remove("visible")}u.forEach(f=>{f.addEventListener("mouseenter",()=>{const b=parseInt(f.getAttribute("data-index"),10);$(b)}),f.addEventListener("mouseleave",k)}),p.forEach(f=>{f.addEventListener("mouseenter",()=>{const b=parseInt(f.getAttribute("data-index"),10);$(b)}),f.addEventListener("mouseleave",k)}),x&&(x.addEventListener("mouseenter",()=>{x.setAttribute("stroke-width","3"),x.setAttribute("opacity","0.9")}),x.addEventListener("mouseleave",()=>{x.setAttribute("stroke-width","2"),x.setAttribute("opacity","0.6")}));const S=document.getElementById("deals-donut-container"),y=document.getElementById("donut-tooltip"),D=document.getElementById("tt-stage"),N=document.getElementById("tt-deals"),T=document.getElementById("tt-percent"),q=document.querySelectorAll(".donut-slice"),dt=document.querySelectorAll(".legend-item");function V(f){if(!y||!S)return;const b=S.getBoundingClientRect(),g=f.clientX-b.left,B=f.clientY-b.top,P=Math.max(75,Math.min(b.width-75,g));B<65?(y.style.left=`${P}px`,y.style.top=`${B+18}px`,y.style.transform="translate(-50%, 0)"):(y.style.left=`${P}px`,y.style.top=`${B-12}px`,y.style.transform="translate(-50%, -100%)")}function z(f,b){q.forEach(g=>{if(g.getAttribute("data-stage")===f){g.classList.add("active-slice"),g.style.opacity="1";const P=g.getAttribute("data-deals"),I=g.getAttribute("data-percent"),E=g.getAttribute("data-color");y&&D&&N&&T&&(D.textContent=f,N.textContent=P,T.textContent=I,T.style.color=E||"var(--color-primary)",y.classList.add("visible"))}else g.classList.remove("active-slice"),g.style.opacity="0.45"}),b&&V(b)}function W(){q.forEach(f=>{f.classList.remove("active-slice"),f.style.opacity="1"}),y&&y.classList.remove("visible")}q.forEach(f=>{f.addEventListener("mouseenter",b=>{z(f.getAttribute("data-stage"),b)}),f.addEventListener("mousemove",b=>{V(b)}),f.addEventListener("mouseleave",W)}),dt.forEach(f=>{f.addEventListener("mouseenter",()=>{z(f.getAttribute("data-stage")),y&&S&&(y.style.left="50%",y.style.top="12px",y.style.transform="translate(-50%, 0)")}),f.addEventListener("mouseleave",W)});const M=document.getElementById("btn-filter-all"),j=document.getElementById("btn-filter-approvals"),pt=document.querySelectorAll(".activity-row"),Q=document.getElementById("activity-empty-row");function J(f){let b=0;pt.forEach(g=>{const B=g.getAttribute("data-is-approval")==="true";f==="all"||f==="approvals"&&B?(g.classList.remove("hidden"),b++):g.classList.add("hidden")}),Q&&(b===0?Q.classList.remove("hidden"):Q.classList.add("hidden"))}M&&j&&(M.addEventListener("click",()=>{M.classList.add("active"),j.classList.remove("active"),J("all")}),j.addEventListener("click",()=>{j.classList.add("active"),M.classList.remove("active"),J("approvals")})),document.querySelectorAll(".btn-review-deal").forEach(f=>{f.addEventListener("click",b=>{const g=f.getAttribute("data-id");g&&(window.location.hash=`#/quotations/${g}`)})});const Y=document.getElementById("btn-dl-audit");Y&&Y.addEventListener("click",()=>{const f=document.querySelectorAll(".activity-row"),b=[["Timestamp","Actor","Role","Customer / Organization","Deal Reference","Valuation","Status"]];f.forEach(E=>{const C=E.querySelectorAll("td");if(C.length>=5){const F=C[0].innerText.trim(),K=C[1].querySelector(".font-bold"),Z=C[1].querySelector(".text-on-surface-variant"),X=C[2].querySelector(".font-semibold"),tt=C[2].querySelector(".font-mono"),ut=C[3].innerText.trim(),mt=C[4].innerText.replace("●","").trim();b.push([`"${F}"`,`"${K?K.innerText.trim():""}"`,`"${Z?Z.innerText.trim():""}"`,`"${X?X.innerText.trim():""}"`,`"${tt?tt.innerText.trim():""}"`,`"${ut}"`,`"${mt}"`])}});const g=b.map(E=>E.join(",")).join(`
`),B=new Blob([g],{type:"text/csv;charset=utf-8;"}),P=URL.createObjectURL(B),I=document.createElement("a");I.setAttribute("href",P),I.setAttribute("download",`dealflow360_audit_trail_${new Date().toISOString().slice(0,10)}.csv`),document.body.appendChild(I),I.click(),document.body.removeChild(I),URL.revokeObjectURL(P)})}function $t(t=[],e="my",n="kanban"){const r=_.getUser()||{full_name:"User"},s=[{id:"draft",title:"Draft",match:o=>o.includes("draft")},{id:"pending",title:"Pending Approval",match:o=>o.includes("pending")||o.includes("review")},{id:"approved",title:"Approved",match:o=>o.includes("approved")},{id:"negotiation",title:"Negotiation",match:o=>o.includes("negotiat")||o.includes("sent")},{id:"confirmed",title:"Confirmed",match:o=>o.includes("confirmed")||o.includes("fulfilled")}],a=o=>{const c=Number(o.total_amount||0).toLocaleString("en-IN");return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
      <div 
...
========
    `},i=s.map(o=>{const c=t.filter(u=>o.match((u.status||"").toLowerCase())),d=c.length===0?'<div class="p-4 text-center text-xs text-on-surface-variant border border-dashed border-outline-variant/40 rounded-2xl">Empty</div>':c.map(a).join("");return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
  `}async function Et(t=!0){try{return await m.get(`/quotations?my_only=${t}`)}catch{return[]}}function kt(t,e){document.querySelectorAll("#quote-scope-toggle button").forEach(s=>{s.addEventListener("click",()=>{const a=s.getAttribute("data-scope");typeof t=="function"&&t(a==="my")})});const r=document.getElementById("btn-mode-toggle");r&&r.addEventListener("click",()=>{const s=r.getAttribute("data-target-mode");typeof e=="function"&&e(s)})}const R={show({title:t,content:e,onConfirm:n,confirmText:r="Confirm",cancelText:s="Cancel",showConfirm:a=!0}){const i=document.getElementById("df-modal-backdrop");i&&i.remove();const l=`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
    ...
    `;document.body.insertAdjacentHTML("beforeend",l);const o=document.getElementById("df-modal-backdrop"),c=document.getElementById("df-modal-close"),d=document.getElementById("df-modal-cancel"),u=document.getElementById("df-modal-confirm"),p=()=>{o.classList.add("fade-out"),setTimeout(()=>o.remove(),200)};return c.addEventListener("click",p),d.addEventListener("click",p),o.addEventListener("click",x=>{x.target===o&&p()}),u&&n&&u.addEventListener("click",async()=>{u.disabled=!0,u.innerHTML='<span class="loading-spinner"></span> Processing...';try{await n()!==!1&&p()}catch(x){alert(x.message||"Action failed")}finally{u.disabled=!1,u.innerHTML=r}}),{close:p}}};function at(t={}){var u,p,x;const{quotation:e=null,products:n=[],customers:r=[]}=t,s=!e||!e.id,a=e||{id:0,deal_reference:"DEAL-NEW",customer_name:((u=r[0])==null?void 0:u.name)||"Acme Corp Global ERP",customer_email:((p=r[0])==null?void 0:p.email)||"procurement@acme.corp",customer_tier:((x=r[0])==null?void 0:x.tier)||"Gold",status:"Draft",lines:[]},i=!a.lines||a.lines.length===0?`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
      <tr>
...
========
      `).join(""),l=a.lines?a.lines.reduce((v,$)=>v+($.line_total||0),0):0,o=l;let c="badge-neutral";const d=(a.status||"").toLowerCase();return d.includes("approved")?c="badge-success":d.includes("pending")?c="badge-warning":d.includes("negotiat")?c="badge-info":(d.includes("fulfilled")||d.includes("confirmed"))&&(c="badge-success"),`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
    <div class="page-container space-y-6">
      <!-- Breadcrumb & Back -->
      <div class="flex items-center justify-between">
...
========
  `}async function st(t){try{const[e,n,r]=await Promise.all([t?m.get(`/quotations/${t}`).catch(()=>null):null,m.get("/products").catch(()=>[]),m.get("/customers").catch(()=>[])]);return{quotation:e,products:n,customers:r}}catch{return{quotation:null,products:[],customers:[]}}}function nt(t,e=[],n=[]){const r=document.getElementById("btn-add-line-modal");r&&r.addEventListener("click",()=>{const u=e.map(p=>`
...
========
        `,confirmText:"Add to Quotation",onConfirm:async()=>{var $;const p=parseInt(document.getElementById("modal-product-select").value,10),x=parseFloat(document.getElementById("modal-product-qty").value)||1;if(!t||t==="0"||t===0||t==="undefined"){const k=(($=n[0])==null?void 0:$.id)||1,S=await m.post("/quotations",{customer_id:k});await m.post(`/quotations/${S.id}/lines`,{product_id:p,quantity:x}),window.location.hash=`#/quotations/${S.id}`,window.location.reload()}else await m.post(`/quotations/${t}/lines`,{product_id:p,quantity:x}),window.location.reload();return!0}})}),document.querySelectorAll(".delete-line-btn").forEach(u=>{u.addEventListener("click",async()=>{const p=u.getAttribute("data-line-id");confirm("Remove this line item from quotation?")&&(await m.delete(`/quotations/${t}/lines/${p}`),window.location.reload())})});const s=async u=>{await m.put(`/quotations/${t}`,{status:u}),window.location.reload()},a=document.getElementById("btn-submit-approval");a&&a.addEventListener("click",()=>s("Pending Approval"));const i=document.getElementById("btn-approve-quote");i&&i.addEventListener("click",()=>s("Approved"));const l=document.getElementById("btn-reject-quote");l&&l.addEventListener("click",()=>s("Rejected"));const o=document.getElementById("btn-send-portal");o&&o.addEventListener("click",()=>s("Under Negotiation"));const c=document.getElementById("btn-confirm-quote");c&&c.addEventListener("click",()=>s("Confirmed"));const d=document.getElementById("btn-fulfill-quote");d&&d.addEventListener("click",()=>s("Fulfilled"))}async function At(){try{return await m.get("/approvals")}catch{return[]}}function _t(t=[]){const e=t.length>0?t:[],n=e.filter(i=>i.status==="PENDING_MANAGER"||i.status==="PENDING_FINANCE").length,r=e.filter(i=>i.status==="RETURNED").length,s=e.filter(i=>i.status==="APPROVED").length,a=e.length===0?`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
      <tr>
...
========
  `}function Lt(){document.querySelectorAll(".approval-row").forEach(a=>{a.addEventListener("click",()=>{const i=a.getAttribute("data-quotation-id");window.location.hash=`#/approval-detail/${i}`})});const t=document.getElementById("btn-filter-pending");if(t){let a=!1;t.addEventListener("click",()=>{a=!a,t.textContent=a?"Show All":"Filter: Pending Only",document.querySelectorAll(".approval-row").forEach(l=>{const o=l.children[3],c=o?o.textContent.trim():"";if(a){const d=c.includes("Sales")||c.includes("Finance")||c==="Pending";l.style.display=d?"":"none"}else l.style.display=""})})}const e=document.getElementById("filter-pending"),n=document.getElementById("filter-returned"),r=document.getElementById("filter-approved");function s(a){document.querySelectorAll(".approval-row").forEach(l=>{const o=l.children[3],c=o?o.textContent.trim():"";if(a==="pending"){const d=c.includes("Sales")||c.includes("Finance")||c==="Pending";l.style.display=d?"":"none"}else a==="returned"?l.style.display=c==="Returned"?"":"none":a==="approved"&&(l.style.display=c==="Approved"||c==="Auto-Approved"?"":"none")})}e&&e.addEventListener("click",()=>s("pending")),n&&n.addEventListener("click",()=>s("returned")),r&&r.addEventListener("click",()=>s("approved"))}async function St(t){try{return await m.get(`/approvals/${t}`)}catch(e){return console.error("Failed to load approval detail",e),null}}function Ct(t){if(!t)return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
      <div class="page-container space-y-6">
...
========
  `,s=["Submitted","Sales Manager","Finance","Confirmed"],a=t.status==="APPROVED"?3:t.status==="PENDING_FINANCE"?2:t.status==="PENDING_MANAGER"?1:0,i=s.map((c,d)=>{let u="#6b7280",p="#6b7280";d<a?(u="#22c55e",p="#22c55e"):d===a&&(u="#3b82f6",p="#3b82f6");const x=d<s.length-1?`<div style="flex: 1; height: 2px; background: ${d<a?"#22c55e":"#6b7280"}; margin: 0 0.25rem;"></div>
      ...
      <div style="display: flex; flex-direction: column; align-items: center; min-width: 80px;">
        <div style="width: 24px; height: 24px; border-radius: 50%; background: ${u}; border: 3px solid ${p};"></div>
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
  `}function It(t){const e=document.getElementById("btn-approve"),n=document.getElementById("btn-return"),r=document.getElementById("btn-reject");e&&e.addEventListener("click",async()=>{const s=prompt("Approval note (optional):")||"";try{await m.post(`/approvals/${t}/decide`,{decision:"APPROVE",reason:s}),alert("Approval granted successfully!"),window.location.hash="#/approvals"}catch(a){alert("Error: "+(a.message||"Approval failed"))}}),n&&n.addEventListener("click",async()=>{const s=prompt("Reason for returning (required):");if(!s){alert("A reason is required to return for revision.");return}try{await m.post(`/approvals/${t}/decide`,{decision:"RETURN",reason:s}),alert("Returned for revision."),window.location.hash="#/approvals"}catch(a){alert("Error: "+(a.message||"Action failed"))}}),r&&r.addEventListener("click",async()=>{const s=prompt("Reason for rejection (required):");if(!s){alert("A reason is required to reject.");return}try{await m.post(`/approvals/${t}/decide`,{decision:"REJECT",reason:s}),alert("Approval rejected."),window.location.hash="#/approvals"}catch(a){alert("Error: "+(a.message||"Action failed"))}})}async function Bt(){try{const t=await m.get("/products");return Array.isArray(t)?t:[]}catch{return[]}}function Pt(t=[]){const e=t.length>0?t:[],n=e.filter(i=>i.is_active!==!1).length,r=e.filter(i=>i.is_active===!1).length,s=new Set(e.map(i=>i.category)),a=e.length===0?`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
      <tr>
...
========
    `:e.map(i=>{const l=Number(i.base_price||0),o=Number(i.tax_rate||0),c=i.unit||"Each",d=i.is_active!==!1,u=d?"badge-success":"badge-error",p=d?"Active":"Archived";return`
...
========
          ...
          <td><span class="badge ${u}">${p}</span></td>
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
        </tr>
      `}).join("");return`
    <div class="page-container space-y-6">
...
========
  `}function Dt(){const t=document.getElementById("btn-new-product");t&&t.addEventListener("click",()=>{window.location.hash="#/product-detail/new"}),document.querySelectorAll(".product-row").forEach(e=>{e.addEventListener("click",()=>{const n=e.getAttribute("data-product-id");window.location.hash=`#/product-detail/${n}`})})}async function Nt(t){if(t==="new")return{product:null,variants:[]};try{const e=await m.get("/products"),n=Array.isArray(e)?e.find(s=>String(s.id)===String(t)):null;let r=[];if(n)try{r=await m.get(`/products/${t}/variants`)}catch{r=[]}return{product:n,variants:Array.isArray(r)?r:[]}}catch{return{product:null,variants:[]}}}function qt(t){const{product:e,variants:n}=t,r=!e,s=(e==null?void 0:e.name)||"",a=(e==null?void 0:e.category)||"",i=(e==null?void 0:e.base_price)||"",l=(e==null?void 0:e.unit)||"",o=(e==null?void 0:e.description)||"",c=(e==null?void 0:e.tax_rate)||"",d="",u={};(n||[]).forEach(x=>{u[x.attribute_name]||(u[x.attribute_name]={values:[],extraPrices:[]}),u[x.attribute_name].values.push(x.value),u[x.attribute_name].extraPrices.push(Number(x.extra_price||0))});const p=Object.keys(u).length>0?Object.entries(u).map(([x,v])=>{const $=v.values.join(", "),k=v.extraPrices.map(y=>y===0?"0":`+$${y}`),S=[...new Set(k)];return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
          <tr>
...
========
  `}function Tt(t){const e=document.getElementById("pd-subscription"),n=document.getElementById("recurring-group");if(e&&n){const a=()=>{n.style.display=e.value==="yes"?"grid":"none"};e.addEventListener("change",a),a()}const r=document.getElementById("btn-add-variant");r&&t&&t!=="new"&&r.addEventListener("click",async()=>{var o,c,d,u,p;const a=(c=(o=document.getElementById("new-attr-name"))==null?void 0:o.value)==null?void 0:c.trim(),i=(u=(d=document.getElementById("new-attr-value"))==null?void 0:d.value)==null?void 0:u.trim(),l=parseFloat((p=document.getElementById("new-attr-price"))==null?void 0:p.value)||0;if(!a||!i){alert("Please fill in attribute name and value.");return}r.disabled=!0;try{await m.post(`/products/${t}/variants`,{attribute_name:a,value:i,extra_price:l}),window.location.hash=`#/product-detail/${t}`,window.dispatchEvent(new HashChangeEvent("hashchange"))}catch(x){alert(x.message||"Failed to add variant"),r.disabled=!1}});const s=document.getElementById("btn-save-product");s&&s.addEventListener("click",async()=>{var p,x,v,$,k,S,y,D,N,T;const a=(x=(p=document.getElementById("pd-name"))==null?void 0:p.value)==null?void 0:x.trim(),i=($=(v=document.getElementById("pd-category"))==null?void 0:v.value)==null?void 0:$.trim(),l=parseFloat((k=document.getElementById("pd-price"))==null?void 0:k.value)||0,o=(y=(S=document.getElementById("pd-unit"))==null?void 0:S.value)==null?void 0:y.trim(),c=parseFloat((D=document.getElementById("pd-tax"))==null?void 0:D.value)||0,d=((T=(N=document.getElementById("pd-description"))==null?void 0:N.value)==null?void 0:T.trim())||null;if(!a||!i||!o){alert("Please fill in product name, category, and unit.");return}const u={name:a,category:i,base_price:l,unit:o,tax_rate:c,description:d};s.disabled=!0,s.innerHTML='<span class="material-symbols-outlined text-lg">hourglass_top</span><span>Saving...</span>';try{if(!t||t==="new"){const q=await m.post("/products",u);window.location.hash=`#/product-detail/${q.id}`}else{try{await m.put(`/products/${t}`,u)}catch{}window.location.hash="#/products"}}catch(q){alert(q.message||"Failed to save product"),s.disabled=!1,s.innerHTML=`<span class="material-symbols-outlined text-lg">save</span><span>${t==="new"?"Create Product":"Save Changes"}</span>`}})}async function Rt(){try{return await m.get("/discount-rules")}catch(t){return console.error("Failed to load discount rules",t),{tiers:[],categories:[],chains:[]}}}function Mt(t){const e=t.tiers||[],n=t.categories||[];t.chains;const r={Bronze:1,Silver:2,Gold:3};e.sort((i,l)=>(r[i.customer_tier]||99)-(r[l.customer_tier]||99));const s=e.map((i,l)=>`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
    <tr>
...
========
  `}async function Ht(){try{return await m.get("/customers")}catch{return[]}}function Ut(){const t=document.getElementById("cust-search-input"),e=document.getElementById("customers-table");t&&e&&t.addEventListener("input",r=>{const s=r.target.value.toLowerCase();e.querySelectorAll("tbody tr").forEach(i=>{i.style.display=i.textContent.toLowerCase().includes(s)?"":"none"})});const n=document.getElementById("btn-add-customer");n&&n.addEventListener("click",()=>{R.show({title:"Add Enterprise Client Account",content:`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
          <div class="space-y-3 text-xs">
            <div>
...
========
        `,confirmText:"Create Account",onConfirm:async()=>{const r=document.getElementById("new-cust-name").value.trim(),s=document.getElementById("new-cust-email").value.trim(),a=document.getElementById("new-cust-tier").value;if(!r||!s)throw new Error("Company name and email are required");return await m.post("/customers",{name:r,email:s,tier:a}),window.location.reload(),!0}})}),document.querySelectorAll(".create-deal-for-cust-btn").forEach(r=>{r.addEventListener("click",async()=>{const s=parseInt(r.getAttribute("data-cust-id"),10);try{const a=await m.post("/quotations",{customer_id:s});window.location.hash=`#/quotations/${a.id}`}catch(a){alert(a.message||"Failed to create quotation")}})})}function Ot(t={}){const{quotation:e=null}=t,n=e||{id:8492,deal_reference:"DEAL-8492",customer_name:"Acme Corp Global ERP",total_amount:34e4,status:"Under Negotiation",lines:[{product_name:"Enterprise Cloud Orchestration Node",quantity:2,unit_price:12e4,line_total:24e4},{product_name:"Architecture Consulting & Migration SLA",quantity:1,unit_price:1e5,line_total:1e5}]},r=["Confirmed","Fulfilled"].includes(n.status);return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
    <div class="page-container space-y-6">
      <!-- Portal Top Banner -->
...
========
  `}async function Qt(t){try{const e=await m.get("/quotations"),n=t?e.find(r=>r.id===parseInt(t,10)):e[0];return n?{quotation:await m.get(`/quotations/${n.id}`).catch(()=>n)}:{quotation:null}}catch{return{quotation:null}}}function Gt(t){const e=(t==null?void 0:t.id)||8492,n=document.getElementById("btn-portal-accept");n&&n.addEventListener("click",async()=>{if(confirm("Confirm digital signature and ratify this commercial agreement?"))try{await m.put(`/quotations/${e}`,{status:"Confirmed"}),alert("Quotation digitally signed and ratified! Deal status updated to Confirmed."),window.location.reload()}catch(i){alert(i.message||"Signature failed")}});const r=document.getElementById("btn-send-portal-msg"),s=document.getElementById("portal-reply-text"),a=document.getElementById("portal-messages-list");r&&s&&a&&r.addEventListener("click",()=>{const i=s.value.trim();if(!i)return;const l=`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
        <div class="p-3 rounded-2xl bg-surface-container-lowest border border-primary-container/60 shadow-sm ml-4">
          <div class="flex items-center justify-between mb-1">
            <span class="font-bold text-secondary">Procurement Lead (Acme Corp)</span>
...
========
  `}async function se(){try{const t=await m.get("/payments/invoices");return Array.isArray(t)?t:[]}catch{return[]}}function ne(){document.querySelectorAll(".download-pdf-btn").forEach(e=>{e.addEventListener("click",async()=>{const n=e.getAttribute("data-id"),r=e.getAttribute("data-number");try{await m.downloadFile(`/payments/invoices/${n}/pdf`,`DealFlow360_Invoice_${r}.pdf`)}catch(s){alert("PDF download failed: "+s.message)}})}),document.querySelectorAll(".settle-btn").forEach(e=>{e.addEventListener("click",()=>{const n=e.getAttribute("data-id");window.location.hash=`#/invoices/${n}`})});const t=document.getElementById("btn-create-invoice");t&&t.addEventListener("click",()=>{alert("Batch invoice generation engine executed: All billable quotation milestones synchronized.")})}function re(t){if(!t||!t.id)return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
    `;const e=(t.status||"").toLowerCase()==="paid",n=t.lines||[],r=n.reduce((d,u)=>d+(Number(u.amount)||0),0)||Number(t.amount)||0,s=r*.065,a=r+s,i=t.payments||[];i.reduce((d,u)=>d+(Number(u.amount)||0),0);const l=e?'<span class="px-3 py-1 rounded-full text-xs font-bold bg-[#E9F3EC] text-[#3D6847] border border-[#C5E2CB]">Paid &amp; Reconciled</span>':'<span class="px-3 py-1 rounded-full text-xs font-bold bg-surface-container-high text-primary">Pending Payment</span>',o=n.map((d,u)=>`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
  `}async function oe(t){try{return await m.get(`/payments/invoices/${t}`)}catch(e){return console.warn("Could not fetch invoice detail:",e),null}}function ie(t){if(!t)return;const e=document.getElementById("btn-download-pdf");e&&e.addEventListener("click",async()=>{try{const s=`DealFlow360_Invoice_${t.number||t.id}.pdf`;await m.downloadFile(`/payments/invoices/${t.id}/pdf`,s)}catch(s){alert("Error generating PDF: "+s.message)}});const n=document.getElementById("btn-send-reminder");n&&n.addEventListener("click",()=>{alert(`Dispatched automated billing reminder to ${t.customer_name||"Buyer"} AP desk.`)});const r=document.getElementById("btn-record-payment");r&&r.addEventListener("click",()=>{const s=r.getAttribute("data-amount");R.show({title:"Record Payment Signoff",content:`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
        `,confirmText:"Execute Payment Signoff",onConfirm:async()=>{var l,o,c;const a=parseFloat((l=document.getElementById("payment-amount-input"))==null?void 0:l.value)||0,i=((c=(o=document.getElementById("payment-ref-input"))==null?void 0:o.value)==null?void 0:c.trim())||"WIRE-DIRECT";try{return await m.post(`/payments/invoices/${t.id}/pay`,{amount:a,reference:i}),alert(`Payment of ₹${a.toLocaleString("en-IN")} recorded successfully!`),window.location.hash=`#/invoices/${t.id}`,window.location.reload(),!0}catch(d){return alert("Failed to record payment: "+d.message),!1}}})})}const le=[{id:1,name:"Enterprise M&A Platform Core",cadence:"yearly",price:12e4,product_id:1},{id:2,name:"Deal Desk Executive Seat",cadence:"monthly",price:350,product_id:2},{id:3,name:"Mission-Critical 24/7 Support SLA",cadence:"yearly",price:45e3,product_id:3}],O=t=>`₹${Number(t||0).toLocaleString("en-IN")}`,ce=t=>(t||"yearly").replace(/^./,e=>e.toUpperCase());function de(t=[]){const e=Array.isArray(t)&&t.length?t:le,n=e.filter(a=>a.is_active!==!1&&a.status!=="Canceled"),r=e.reduce((a,i)=>a+Number(i.price||0)/((i.cadence||"").toLowerCase()==="monthly"?1:12),0),s=e.map((a,i)=>{const l=ce(a.cadence),o=a.is_active===!1||a.status==="Canceled"?"Canceled":"Active";return`<a class="subscription-row" href="#/subscriptions/${a.id}" data-search="${`${a.name} ${a.id} ${a.product_id}`.toLowerCase()}">
      ...
      <div><strong class="font-mono text-sm text-on-surface">${O(a.price)} <span class="text-[11px] font-sans text-on-surface-variant">/ ${l==="Monthly"?"mo":"yr"}</span></strong><span class="block text-[11px] text-on-surface-variant">${l} recurring billing</span></div>
    ...
    <section class="card card-extruded space-y-4"><div class="flex items-center gap-2"><span class="material-symbols-outlined text-primary">calculate</span><div><h3 class="text-base font-bold">Mid-Cycle Seat Upgrade &amp; Proration</h3><p class="text-xs text-on-surface-variant">Preview the delta before changing a recurring contract.</p></div></div><div class="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs"><label>Active plan<select class="input-clay w-full mt-1" id="prorate-plan"><option value="4200">Executive Seats (₹4,200/yr)</option><option value="120000">Enterprise Core (₹120k/yr)</option></select></label><label>Seats added<input type="number" id="prorate-seats" min="1" value="5" class="input-clay w-full mt-1" /></label><label>Days remaining<input type="number" id="prorate-days" min="1" max="365" value="142" class="input-clay w-full mt-1" /></label><label>Proration delta<div class="font-mono font-bold text-base text-primary p-2 mt-1 rounded-lg bg-surface-container" id="prorate-result">₹8,169.86</div></label></div></section>
  </div>`}async function pe(){try{return await m.get("/subscriptions/plans")}catch{return[]}}function ue(){var u;const t=document.getElementById("subscription-search"),e=[...document.querySelectorAll(".subscription-row")],n=document.getElementById("subscription-empty"),r=[...document.querySelectorAll(".subscription-filter")];let s="all";const a=()=>{const p=((t==null?void 0:t.value)||"").toLowerCase().trim();let x=0;e.forEach(v=>{const $=!p||v.dataset.search.includes(p),k=s!=="active"||v.querySelector(".badge-success");v.classList.toggle("hidden",!$||!k),$&&k&&(x+=1)}),n==null||n.classList.toggle("hidden",x>0)};t==null||t.addEventListener("input",a),r.forEach(p=>p.addEventListener("click",()=>{s=p.dataset.filter,r.forEach(x=>x.classList.toggle("active",x===p)),a()}));const i=document.getElementById("prorate-seats"),l=document.getElementById("prorate-days"),o=document.getElementById("prorate-plan"),c=document.getElementById("prorate-result"),d=()=>{c&&(c.textContent=O(Number(o==null?void 0:o.value)/365*Number((l==null?void 0:l.value)||0)*Number((i==null?void 0:i.value)||0)))};[i,l,o].forEach(p=>p==null?void 0:p.addEventListener("input",d)),(u=document.getElementById("btn-add-plan"))==null||u.addEventListener("click",()=>R.show({title:"New Subscription Plan",content:'<input id="plan-name-in" type="text" class="input-clay w-full" placeholder="Plan name" /><input id="plan-price-in" type="number" class="input-clay w-full mt-3" placeholder="Base rate (INR)" />',confirmText:"Create Plan",onConfirm:async()=>{const p=document.getElementById("plan-name-in").value.trim(),x=Number(document.getElementById("plan-price-in").value);if(!p||!x)throw new Error("Plan name and price are required");return await m.post("/subscriptions/plans",{name:p,cadence:"yearly",product_id:1,price:x}),window.location.reload(),!0}}))}const U=t=>`₹${Number(t||0).toLocaleString("en-IN",{minimumFractionDigits:2})}`,rt=t=>(t||"yearly").replace(/^./,e=>e.toUpperCase());function me(t){var i;if(!(t!=null&&t.id))return'<div class="page-container flex items-center justify-center min-h-[60vh]"><div class="card card-extruded p-8 text-center"><span class="material-symbols-outlined text-4xl text-outline mb-3">error_outline</span><h2 class="text-lg font-bold">Subscription Not Found</h2><a href="#/subscriptions" class="btn btn-primary text-xs mt-4">Back to Subscriptions</a></div></div>';const e=t.schedules||[],n=e.filter(l=>l.status==="Billed").reduce((l,o)=>l+Number(o.amount||0),0),r=e.filter(l=>l.status==="Scheduled").reduce((l,o)=>l+Number(o.amount||0),0),s=e.length?e.map(l=>{var o;return`<tr class="border-b border-surface-container-high/40"><td class="py-3 px-4 font-mono text-xs">${l.projected?"PROJECTED":`BILL-${String(l.id).padStart(5,"0")}`}</td><td class="py-3 px-4 font-mono text-xs">${l.billing_date||"Scheduled"}</td><td class="py-3 px-4 text-xs">${l.customer_name||((o=t.customers)==null?void 0:o[0])||"Unassigned account"}</td><td class="py-3 px-4 font-mono text-xs text-right font-bold">${U(l.amount)}</td><td class="py-3 px-4"><span class="badge ${l.status==="Billed"?"badge-success":"badge-warning"} text-[10px]">${l.status||"Scheduled"}</span></td></tr>`}).join(""):'<tr><td colspan="5" class="py-8 text-center text-xs text-on-surface-variant">No billing cycles have been generated for this subscription yet.</td></tr>',a=t.is_active!==!1;return`<div class="page-container space-y-6"><div class="flex items-center gap-3"><a href="#/subscriptions" class="btn btn-secondary text-xs py-1.5 px-3"><span class="material-symbols-outlined text-sm">arrow_back</span><span>Back to Subscriptions</span></a><span class="text-xs text-outline">/</span><span class="text-xs font-mono font-bold text-primary">SUB-${String(t.id).padStart(4,"0")}</span></div>
    <div class="card card-extruded"><div class="flex flex-col lg:flex-row lg:items-start justify-between gap-4"><div class="flex items-center gap-4"><div class="w-14 h-14 rounded-2xl bg-surface-container flex items-center justify-center text-primary"><span class="material-symbols-outlined text-3xl">receipt_long</span></div><div><div class="flex items-center gap-2 flex-wrap"><h1 class="text-xl font-bold text-on-surface">Billing Detail</h1><span class="badge ${a?"badge-success":"badge-error"} text-[10px]">${a?"Active":"Canceled"}</span></div><p class="text-sm font-bold text-on-surface mt-1">${t.name}</p><p class="text-xs text-on-surface-variant mt-1">Product ${t.product_id} · ${rt(t.cadence)} recurring contract</p></div></div><div class="flex items-center gap-2">${a?'<button type="button" class="btn btn-secondary text-xs" id="btn-modify-subscription"><span class="material-symbols-outlined text-sm">edit_note</span><span>Modify Terms</span></button><button type="button" class="btn btn-secondary text-xs text-error" id="btn-cancel-subscription"><span class="material-symbols-outlined text-sm">bolt</span><span>Cancel Contract</span></button>':""}</div></div></div>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4"><div class="card card-extruded subscription-detail-stat"><span>Contract Value</span><strong>${U(t.price)}</strong><em>per ${rt(t.cadence).toLowerCase()} cycle</em></div><div class="card card-extruded subscription-detail-stat"><span>Billed To Date</span><strong>${U(n)}</strong><em>${e.filter(l=>l.status==="Billed").length} settled cycles</em></div><div class="card card-extruded subscription-detail-stat"><span>Scheduled Balance</span><strong>${U(r)}</strong><em>${e.filter(l=>l.status==="Scheduled").length} upcoming cycles</em></div></div>
    ...
    <section class="card card-extruded"><div class="flex items-center gap-2 mb-3"><span class="material-symbols-outlined text-primary">account_balance</span><h2 class="text-base font-bold">Remittance &amp; Contract Context</h2></div><div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs"><div><span class="detail-label">Customer entity</span><strong>${((i=t.customers)==null?void 0:i.join(", "))||"Unassigned account"}</strong></div><div><span class="detail-label">Payment terms</span><strong>Net 30 · Direct wire</strong></div><div><span class="detail-label">Ledger status</span><strong class="text-primary">Reconciled and monitored</strong></div></div></section></div>`}async function fe(t){try{return await m.get(`/subscriptions/plans/${t}/billing-detail`)}catch(e){return console.warn("Could not fetch subscription billing detail:",e),null}}function xe(t){var e,n;(e=document.getElementById("btn-modify-subscription"))==null||e.addEventListener("click",()=>R.show({title:"Modify Subscription Terms",content:`<div class="space-y-3 text-xs"><label class="block font-bold">Plan name<input id="modify-plan-name" class="input-clay w-full mt-1" value="${t.name}" /></label><label class="block font-bold">Recurring price (INR)<input id="modify-plan-price" type="number" min="0.01" class="input-clay w-full mt-1" value="${t.price}" /></label><label class="block font-bold">Cadence<select id="modify-plan-cadence" class="input-clay w-full mt-1"><option value="monthly" ${t.cadence==="monthly"?"selected":""}>Monthly</option><option value="quarterly" ${t.cadence==="quarterly"?"selected":""}>Quarterly</option><option value="yearly" ${t.cadence==="yearly"?"selected":""}>Yearly</option></select></label></div>`,confirmText:"Save Terms",onConfirm:async()=>(await m.patch(`/subscriptions/plans/${t.id}`,{name:document.getElementById("modify-plan-name").value.trim(),price:Number(document.getElementById("modify-plan-price").value),cadence:document.getElementById("modify-plan-cadence").value}),window.location.hash=`#/subscriptions/${t.id}`,window.location.reload(),!0)})),(n=document.getElementById("btn-cancel-subscription"))==null||n.addEventListener("click",()=>{var s,a;const r=(a=(s=t==null?void 0:t.schedules)==null?void 0:s.find(i=>i.quotation_line_id))==null?void 0:a.quotation_line_id;R.show({title:"Cancel Subscription",content:'<p class="text-xs text-on-surface-variant">Future billing cycles will stop and this contract will be marked canceled.</p>',confirmText:"Cancel Contract",onConfirm:async()=>(r?await m.post(`/subscriptions/lines/${r}/cancel`,{}):await m.post(`/subscriptions/plans/${t.id}/cancel`,{}),window.location.hash="#/subscriptions",window.location.reload(),!0)})})}let A={period:"This Month",status:"All Statuses",product:"All Products"};async function it(t={}){try{const e=new URLSearchParams(t).toString();return{kpis:await m.get(`/reports/kpis?${e}`).catch(()=>null)}}catch{return{kpis:null}}}function ve(t={}){const e=t.kpis||{quotes_created:0,avg_approval_time_hours:0,top_upsold_product:"None"},n=A.period.toLowerCase();return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
                  <span class="text-[11px] font-bold text-on-surface">${(_.getUser()||{full_name:"User"}).full_name||"You"}</span>
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
                  <span class="text-[10px] text-on-surface-variant font-mono">10:42 AM</span>
                </div>
                <div class="bg-surface-container p-3 rounded-2xl rounded-tl-xs text-xs text-on-surface border border-surface-container-high/60">
...
========
  `}function ye(){const t=document.getElementById("btn-send-msg"),e=document.getElementById("msg-input"),n=document.getElementById("messages-list");if(t&&e&&n){const r=()=>{const s=e.value.trim();if(!s)return;const a=_.getUser()||{full_name:"You"},i=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),l=document.createElement("div");l.className="flex flex-col items-start max-w-[80%]",l.innerHTML=`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
        <div class="flex items-center gap-2 mb-1">
...
========
      `,n.appendChild(l),e.value="",n.scrollTop=n.scrollHeight};t.addEventListener("click",r),e.addEventListener("keydown",s=>{s.key==="Enter"&&r()})}}function he(){const t=_.getUser()||{full_name:"Alice Johnson",email:"salesrep@dealflow360.com",role:"SalesRep",phone:"+1 (555) 234-5678",address:"742 Evergreen Terrace, San Francisco, CA 94107",age:"28"},e=t.full_name?t.full_name.split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2):"US";return`
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
...
========
  `}function we(){const t=document.getElementById("profile-form"),e=document.getElementById("prof-name"),n=document.getElementById("prof-phone"),r=document.getElementById("prof-age"),s=document.getElementById("prof-address"),a=document.getElementById("profile-msg"),i=document.getElementById("btn-profile-logout");t&&t.addEventListener("submit",l=>{l.preventDefault();const o=e.value.trim();if(!o)return;const c=_.getUser()||{};c.full_name=o,c.phone=n?n.value.trim():c.phone,c.age=r?r.value.trim():c.age,c.address=s?s.value.trim():c.address,_.setUser(c),a&&(a.classList.remove("hidden"),setTimeout(()=>a.classList.add("hidden"),3500))}),i&&i.addEventListener("click",()=>{confirm("Are you sure you want to sign out of DealFlow360?")&&_.logout()})}async function $e(){return await L.fetchHealthData(),{}}function lt(){const t=L.getStalledDeals().length,e=L.getDiscountAnomalies().length,n=L.getDeliverySlippages().length,r=L.getFlaggedItems(),s=r.length>0?r.map(a=>{let i="badge-warning";return a.actionStatus==="Nudge sent"?i="badge-info":a.actionStatus==="Escalated to Manager"?i="badge-primary":a.actionStatus==="Resolved"&&(i="badge-success"),`
...
========
    `}async handleRoute(){const{route:e,param:n,query:r}=this.parseHash(),s=_.isAuthenticated();if(!s&&e!=="login"&&e!=="signup"&&e!=="portal"){window.location.hash="#/login";return}if(s&&(e==="login"||e==="signup")){window.location.hash="#/quotations";return}const a=_.getUser(),i=a?a.selected_role||a.role:"",l={SalesRep:["dashboard","quotations","customers","products","profile"],SalesManager:["dashboard","quotations","approvals","pricing","reports","profile"],FinanceOps:["dashboard","approvals","fulfillment","invoices","subscriptions","profile"],Admin:["dashboard","quotations","approvals","deal-health","fulfillment","invoices","customers","products","pricing","subscriptions","reports","profile"],Customer:["quotations","messages","profile"]};if(!["login","signup","portal"].includes(e)&&!(l[i]||l.SalesRep).includes(e)){window.location.hash="#/quotations";return}switch(window.scrollTo(0,0),e){case"login":{this.appEl.innerHTML=ft(),xt();break}case"signup":{this.appEl.innerHTML=vt(),bt();break}case"dashboard":{this.showLoading();const o=await ht();this.appEl.innerHTML=`
          ${h("dashboard")}
          <main class="main-content">${yt(o)}</main>
        `,w(),wt();break}case"deal-health":{this.showLoading(),await $e(),this.appEl.innerHTML=`
          ${h("deal-health")}
          <main class="main-content" id="app-content">${lt()}</main>
        `,w(),ct();break}case"quotations":{if(n){this.showLoading();const o=await st(n);this.appEl.innerHTML=`
            ${h("quotations")}
            <main class="main-content">${at(o)}</main>
          `,w(),nt(n,o.products,o.customers)}else{this.showLoading();let o=!0,c="kanban";const d=async(u=o,p=c)=>{o=u,c=p;const x=await Et(u);this.appEl.innerHTML=`
              ${h("quotations")}
              <main class="main-content">${$t(x,u?"my":"all",p)}</main>
            `,w(),kt(v=>d(v,c),v=>d(o,v))};await d(!0,"kanban")}break}case"quotation-detail":{this.showLoading();const o=n||r.get("id"),c=await st(o);this.appEl.innerHTML=`
          ${h("quotations")}
          <main class="main-content">${at(c)}</main>
        `,w(),nt(o,c.products,c.customers);break}case"approvals":{this.showLoading();const o=await At();this.appEl.innerHTML=`
          ${h("approvals")}
          <main class="main-content">${_t(o)}</main>
        `,w(),Lt();break}case"approval-detail":{this.showLoading();const o=n||r.get("id"),c=await St(o);this.appEl.innerHTML=`
          ${h("approvals")}
          <main class="main-content">${Ct(c)}</main>
        `,w(),It(o);break}case"products":{this.showLoading();const o=await Bt();this.appEl.innerHTML=`
          ${h("products")}
          <main class="main-content">${Pt(o)}</main>
        `,w(),Dt();break}case"product-detail":{this.showLoading();const o=n||"new",c=await Nt(o);this.appEl.innerHTML=`
          ${h("products")}
          <main class="main-content">${qt(c)}</main>
        `,w(),Tt(o);break}case"pricing":case"pricing-rules":{this.showLoading();const o=await Rt();this.appEl.innerHTML=`
          ${h("pricing")}
          <main class="main-content">${Mt(o)}</main>
        `,w(),jt();break}case"customers":{this.showLoading();const o=await Ht();this.appEl.innerHTML=`
          ${h("customers")}
          <main class="main-content">${Ft(o)}</main>
        `,w(),Ut();break}case"portal":{this.showLoading();const o=n||r.get("id"),c=await Qt(o);this.appEl.innerHTML=`
          <main class="main-content portal-shell">${Ot(c)}</main>
        `,Gt(c.quotation);break}case"fulfillment":{if(this.showLoading(),n){const o=await Kt(n);this.appEl.innerHTML=`${h("fulfillment")}<main class="main-content">${Yt(o)}</main>`,w(),Zt(n)}else{const o=await Wt();this.appEl.innerHTML=`${h("fulfillment")}<main class="main-content">${zt(o)}</main>`,w(),Jt()}break}case"fulfillment-override":{this.showLoading();const o=await te(n);this.appEl.innerHTML=`${h("fulfillment")}<main class="main-content">${Xt(o)}</main>`,w(),ee(n);break}case"invoices":{if(n){this.showLoading();const o=await oe(n);this.appEl.innerHTML=`
            ${h("invoices")}
            <main class="main-content">${re(o)}</main>
          `,w(),ie(o)}else{this.showLoading();const o=await se();this.appEl.innerHTML=`
            ${h("invoices")}
            <main class="main-content">${ae(o)}</main>
          `,w(),ne()}break}case"subscriptions":{if(this.showLoading(),n){const o=await fe(n);this.appEl.innerHTML=`${h("subscriptions")}<main class="main-content">${me(o)}</main>`,w(),xe(o)}else{const o=await pe();this.appEl.innerHTML=`${h("subscriptions")}<main class="main-content">${de(o)}</main>`,w(),ue()}break}case"reports":{this.showLoading();const o=await it();this.appEl.innerHTML=`
          ${h("reports")}
          <main class="main-content">${ve(o)}</main>
        `,w(),be();break}case"messages":{this.appEl.innerHTML=`
          ${h("messages")}
          <main class="main-content">${ge()}</main>
        `,w(),ye();break}case"profile":{this.appEl.innerHTML=`
          ${h("profile")}
          <main class="main-content">${he()}</main>
        `,w(),we();break}default:{window.location.hash="#/quotations";break}}}}function ot(){m.setToken(null),_.setUser(null),window.location.hash!=="#/login"&&window.history.replaceState(null,"",`${window.location.pathname}${window.location.search}#/login`),new Ee().init()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ot,{once:!0}):ot();
>>>>>>>> 9fd844c (Add different LogIn):frontend/dist/assets/index-CEhWhULA.js
