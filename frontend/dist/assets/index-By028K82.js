(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))r(a);new MutationObserver(a=>{for(const e of a)if(e.type==="childList")for(const o of e.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function n(a){const e={};return a.integrity&&(e.integrity=a.integrity),a.referrerPolicy&&(e.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?e.credentials="include":a.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function r(a){if(a.ep)return;a.ep=!0;const e=n(a);fetch(a.href,e)}})();const I="/api/v1";class _ extends Error{constructor(s,n,r){super(s),this.name="ApiError",this.status=n,this.data=r}}const p={getToken(){return localStorage.getItem("dealflow_token")},setToken(t){t?localStorage.setItem("dealflow_token",t):localStorage.removeItem("dealflow_token")},getHeaders(t={}){const s={"Content-Type":"application/json",...t},n=this.getToken();return n&&(s.Authorization=`Bearer ${n}`),s},async request(t,s={}){const n=`${I}${t}`,r=this.getHeaders(s.headers),a={...s,headers:r};a.body&&typeof a.body=="object"&&!(a.body instanceof FormData)&&(a.body=JSON.stringify(a.body));try{const e=await fetch(n,a);if(e.status===401&&!t.includes("/auth/login"))throw this.setToken(null),localStorage.removeItem("dealflow_user"),window.location.hash="#/login",new _("Session expired. Please sign in again.",401,null);if(e.status===204)return null;const o=e.headers.get("content-type")||"";let i=null;if(o.includes("application/json")?i=await e.json():i=await e.text(),!e.ok){const l=(i==null?void 0:i.detail)||(i==null?void 0:i.message)||`Request failed with status ${e.status}`;throw new _(l,e.status,i)}return i}catch(e){throw e instanceof _?e:new _(e.message||"Network connection failed",0,null)}},get(t,s){let n=t;if(s){const r=new URLSearchParams(s).toString();r&&(n+=`?${r}`)}return this.request(n,{method:"GET"})},post(t,s){return this.request(t,{method:"POST",body:s})},put(t,s){return this.request(t,{method:"PUT",body:s})},patch(t,s){return this.request(t,{method:"PATCH",body:s})},delete(t){return this.request(t,{method:"DELETE"})},async downloadFile(t,s){const n=`${I}${t}`,r=this.getToken(),a=r?{Authorization:`Bearer ${r}`}:{},e=await fetch(n,{headers:a});if(!e.ok)throw new Error(`Download failed with status ${e.status}`);const o=await e.blob(),i=window.URL.createObjectURL(o),l=document.createElement("a");l.href=i,l.download=s||"document.pdf",document.body.appendChild(l),l.click(),l.remove(),window.URL.revokeObjectURL(i)}},y={getUser(){try{const t=localStorage.getItem("dealflow_user");return t?JSON.parse(t):null}catch{return null}},setUser(t){t?localStorage.setItem("dealflow_user",JSON.stringify(t)):localStorage.removeItem("dealflow_user")},isAuthenticated(){return!!p.getToken()},async login(t,s,n){const r=await p.post("/auth/login",{email:t,password:s});if(r.access_token){p.setToken(r.access_token);try{const a=await p.get("/auth/me");return a.selected_role=n||a.role,this.setUser(a),{success:!0,user:a}}catch{const a={email:t,full_name:t.split("@")[0],role:n||"SalesRep",selected_role:n||"SalesRep"};return this.setUser(a),{success:!0,user:a}}}throw new Error("Authentication failed: No access token received")},async signup({full_name:t,email:s,password:n,role:r}){return await p.post("/auth/signup",{full_name:t,email:s,password:n,role:r})},logout(){p.setToken(null),this.setUser(null),window.location.hash="#/login"}};function b(t="quotations"){const s=y.getUser()||{full_name:"User",role:"SalesRep"},n=s.selected_role||s.role||"",r=[{key:"dashboard",label:"Dashboard",icon:"dashboard",href:"#/dashboard"},{key:"quotations",label:"Quotations",icon:"request_quote",href:"#/quotations"},{key:"approvals",label:"Approvals",icon:"verified",href:"#/approvals"},{key:"fulfillment",label:"Fulfillment",icon:"assignment_turned_in",href:"#/fulfillment"},{key:"invoices",label:"Invoices",icon:"receipt_long",href:"#/invoices"},{key:"products",label:"Products",icon:"inventory_2",href:"#/products"},{key:"pricing",label:"Pricing",icon:"sell",href:"#/pricing"},{key:"subscriptions",label:"Subscriptions",icon:"sync",href:"#/subscriptions"},{key:"reports",label:"Reports",icon:"bar_chart",href:"#/reports"},{key:"messages",label:"Messages",icon:"chat_bubble",href:"#/messages"},{key:"profile",label:"Profile",icon:"account_circle",href:"#/profile"}];return n==="Admin"&&r.unshift({key:"dashboard",label:"Dashboard",icon:"dashboard",href:"#/dashboard"}),`
    <header class="navbar-header">
      <div class="navbar-inner">
        ...
        <nav class="navbar-nav">
          ${r.map(e=>{const i=t===e.key||t==="quotation-detail"&&e.key==="quotations"?"nav-item-active":"nav-item-inactive";return`
  ...
  `}function M(){const t=document.getElementById("login-form"),s=document.getElementById("login-error"),n=document.getElementById("btn-submit-login"),r=document.getElementById("toggle-pw"),a=document.getElementById("login-password"),e=document.getElementById("pw-icon");r&&a&&e&&r.addEventListener("click",()=>{a.type==="password"?(a.type="text",e.textContent="visibility_off"):(a.type="password",e.textContent="visibility")}),t&&t.addEventListener("submit",async o=>{o.preventDefault(),s.classList.add("hidden"),s.textContent="",n.disabled=!0,n.innerHTML='<span class="loading-spinner"></span> Authenticating...';const i=document.getElementById("login-email").value.trim(),l=a.value.trim();try{await y.login(i,l),window.location.hash="#/quotations"}catch(c){s.textContent=c.message||"Login failed. Check your credentials.",s.classList.remove("hidden")}finally{n.disabled=!1,n.innerHTML='<span><span>Sign In to DealFlow360</span><span class="material-symbols-outlined text-base">arrow_forward</span></span>'}})}function H(){return`
    <div class="login-wrapper flex items-center justify-center min-h-screen w-full p-4">
      <!-- Ambient clay orbs -->
      <div class="ambient-orb orb-1"></div>
      <div class="ambient-orb orb-2"></div>
  ...
  `}function F(){const t=document.getElementById("signup-form"),s=document.getElementById("signup-error"),n=document.getElementById("signup-success"),r=document.getElementById("btn-submit-signup"),a=document.getElementById("signup-toggle-pw"),e=document.getElementById("signup-password"),o=document.getElementById("signup-pw-icon");a&&e&&o&&a.addEventListener("click",()=>{e.type==="password"?(e.type="text",o.textContent="visibility_off"):(e.type="password",o.textContent="visibility")}),t&&t.addEventListener("submit",async i=>{i.preventDefault(),s.classList.add("hidden"),s.textContent="",n.classList.add("hidden"),n.textContent="",r.disabled=!0,r.innerHTML='<span class="loading-spinner"></span> Creating Account...';const l=document.getElementById("signup-name").value.trim(),c=document.getElementById("signup-email").value.trim(),d=e.value,u=document.getElementById("signup-confirm-password").value;if(d!==u){s.textContent="Passwords do not match.",s.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<span>Create Account</span><span class="material-symbols-outlined text-base">person_add</span>';return}if(d.length<8){s.textContent="Password must be at least 8 characters.",s.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<span>Create Account</span><span class="material-symbols-outlined text-base">person_add</span>';return}try{await y.signup({full_name:l,email:c,password:d,role:"SalesRep"}),n.textContent="Account created successfully! Redirecting to login...",n.classList.remove("hidden"),t.reset(),setTimeout(()=>{window.location.hash="#/login"},1500)}catch(m){s.textContent=m.message||"Signup failed. Please try again.",s.classList.remove("hidden")}finally{r.disabled=!1,r.innerHTML='<span>Create Account</span><span class="material-symbols-outlined text-base">person_add</span>'}})}const E={show({title:t,content:s,onConfirm:n,confirmText:r="Confirm",cancelText:a="Cancel",showConfirm:e=!0}){const o=document.getElementById("df-modal-backdrop");o&&o.remove();const i=`
  ...
  `}async function O(){try{const[t,s]=await Promise.all([p.get("/dashboard/summary").catch(()=>({})),p.get("/quotations").catch(()=>[])]);return{summary:t,quotations:s}}catch{return{summary:{},quotations:[]}}}function Q(){const t=document.getElementById("btn-export-dash");t&&t.addEventListener("click",()=>{window.location.hash="#/reports"});const s=document.getElementById("btn-new-deal-dash");s&&s.addEventListener("click",()=>{window.location.hash="#/quotation-detail"});const n=document.getElementById("btn-dl-audit");n&&n.addEventListener("click",()=>{alert("Downloading audit trail CSV...")})}function G(t=[],s="my",n="kanban"){const r=y.getUser()||{full_name:"User"},a=[{id:"draft",title:"Draft",match:l=>l.includes("draft")},{id:"pending",title:"Pending Approval",match:l=>l.includes("pending")||l.includes("review")},{id:"approved",title:"Approved",match:l=>l.includes("approved")},{id:"negotiation",title:"Negotiation",match:l=>l.includes("negotiat")||l.includes("sent")},{id:"confirmed",title:"Confirmed",match:l=>l.includes("confirmed")||l.includes("fulfilled")}],e=l=>{const c=Number(l.total_amount||0).toLocaleString("en-IN");return`
    ...
    `},o=a.map(l=>{const c=t.filter(u=>l.match((u.status||"").toLowerCase())),d=c.length===0?'<div class="p-4 text-center text-xs text-on-surface-variant border border-dashed border-outline-variant/40 rounded-2xl">Empty</div>':c.map(e).join("");return`
  ...
  `}async function V(t=!0){try{return await p.get(`/quotations?my_only=${t}`)}catch{return[]}}function z(t,s){document.querySelectorAll("#quote-scope-toggle button").forEach(a=>{a.addEventListener("click",()=>{const e=a.getAttribute("data-scope");typeof t=="function"&&t(e==="my")})});const r=document.getElementById("btn-mode-toggle");r&&r.addEventListener("click",()=>{const a=r.getAttribute("data-target-mode");typeof s=="function"&&s(a)})}function N(t={}){var u,m,x;const{quotation:s=null,products:n=[],customers:r=[]}=t,a=!s||!s.id,e=s||{id:0,deal_reference:"DEAL-NEW",customer_name:((u=r[0])==null?void 0:u.name)||"Acme Corp Global ERP",customer_email:((m=r[0])==null?void 0:m.email)||"procurement@acme.corp",customer_tier:((x=r[0])==null?void 0:x.tier)||"Gold",status:"Draft",lines:[]},o=!e.lines||e.lines.length===0?`
      <tr>
  ...
  `}async function D(t){try{const[s,n,r]=await Promise.all([t?p.get(`/quotations/${t}`).catch(()=>null):null,p.get("/products").catch(()=>[]),p.get("/customers").catch(()=>[])]);return{quotation:s,products:n,customers:r}}catch{return{quotation:null,products:[],customers:[]}}}function T(t,s=[],n=[]){const r=document.getElementById("btn-add-line-modal");r&&r.addEventListener("click",()=>{const u=s.map(m=>`
      ...
      `).join("");E.show({title:"Add Product Line Item",content:`
        ...
        `,confirmText:"Add to Quotation",onConfirm:async()=>{var h;const m=parseInt(document.getElementById("modal-product-select").value,10),x=parseFloat(document.getElementById("modal-product-qty").value)||1;if(!t||t==="0"||t===0||t==="undefined"){const w=((h=n[0])==null?void 0:h.id)||1,$=await p.post("/quotations",{customer_id:w});await p.post(`/quotations/${$.id}/lines`,{product_id:m,quantity:x}),window.location.hash=`#/quotations/${$.id}`,window.location.reload()}else await p.post(`/quotations/${t}/lines`,{product_id:m,quantity:x}),window.location.reload();return!0}})}),document.querySelectorAll(".delete-line-btn").forEach(u=>{u.addEventListener("click",async()=>{const m=u.getAttribute("data-line-id");confirm("Remove this line item from quotation?")&&(await p.delete(`/quotations/${t}/lines/${m}`),window.location.reload())})});const a=async u=>{await p.put(`/quotations/${t}`,{status:u}),window.location.reload()},e=document.getElementById("btn-submit-approval");e&&e.addEventListener("click",()=>a("Pending Approval"));const o=document.getElementById("btn-approve-quote");o&&o.addEventListener("click",()=>a("Approved"));const i=document.getElementById("btn-reject-quote");i&&i.addEventListener("click",()=>a("Rejected"));const l=document.getElementById("btn-send-portal");l&&l.addEventListener("click",()=>a("Under Negotiation"));const c=document.getElementById("btn-confirm-quote");c&&c.addEventListener("click",()=>a("Confirmed"));const d=document.getElementById("btn-fulfill-quote");d&&d.addEventListener("click",()=>a("Fulfilled"))}async function W(){try{return await p.get("/approvals")}catch{return[]}}function Y(t=[]){const s=t.length>0?t:[],n=s.filter(o=>o.status==="PENDING_MANAGER"||o.status==="PENDING_FINANCE").length,r=s.filter(o=>o.status==="RETURNED").length,a=s.filter(o=>o.status==="APPROVED").length,e=s.length===0?`
      <tr>
    ...
    `:s.map(o=>{const i=o.blended_risk==="HIGH"?"font-bold":"";return`
  ...
  `}function J(){document.querySelectorAll(".approval-row").forEach(e=>{e.addEventListener("click",()=>{const o=e.getAttribute("data-quotation-id");window.location.hash=`#/approval-detail/${o}`})});const t=document.getElementById("btn-filter-pending");if(t){let e=!1;t.addEventListener("click",()=>{e=!e,t.textContent=e?"Show All":"Filter: Pending Only",document.querySelectorAll(".approval-row").forEach(i=>{const l=i.children[3],c=l?l.textContent.trim():"";if(e){const d=c.includes("Sales")||c.includes("Finance")||c==="Pending";i.style.display=d?"":"none"}else i.style.display=""})})}const s=document.getElementById("filter-pending"),n=document.getElementById("filter-returned"),r=document.getElementById("filter-approved");function a(e){document.querySelectorAll(".approval-row").forEach(i=>{const l=i.children[3],c=l?l.textContent.trim():"";if(e==="pending"){const d=c.includes("Sales")||c.includes("Finance")||c==="Pending";i.style.display=d?"":"none"}else e==="returned"?i.style.display=c==="Returned"?"":"none":e==="approved"&&(i.style.display=c==="Approved"||c==="Auto-Approved"?"":"none")})}s&&s.addEventListener("click",()=>a("pending")),n&&n.addEventListener("click",()=>a("returned")),r&&r.addEventListener("click",()=>a("approved"))}async function K(t){try{return await p.get(`/approvals/${t}`)}catch(s){return console.error("Failed to load approval detail",s),null}}function X(t){if(!t)return`
      <div class="page-container space-y-6">
    ...
    `;const s=t.blended_risk==="HIGH"?"#e67e22":t.blended_risk==="MEDIUM"?"#f1c40f":"#2ecc71",n="#5AA1E3",r=(t.lines||[]).map(c=>`
    <tr>
  ...
  `,a=["Submitted","Sales Manager","Finance","Confirmed"],e=t.status==="APPROVED"?3:t.status==="PENDING_FINANCE"?2:t.status==="PENDING_MANAGER"?1:0,o=a.map((c,d)=>{let u="#6b7280",m="#6b7280";d<e?(u="#22c55e",m="#22c55e"):d===e&&(u="#3b82f6",m="#3b82f6");const x=d<a.length-1?`<div style="flex: 1; height: 2px; background: ${d<e?"#22c55e":"#6b7280"}; margin: 0 0.25rem;"></div>
  ...
  `}function Z(t){const s=document.getElementById("btn-approve"),n=document.getElementById("btn-return"),r=document.getElementById("btn-reject");s&&s.addEventListener("click",async()=>{const a=prompt("Approval note (optional):")||"";try{await p.post(`/approvals/${t}/decide`,{decision:"APPROVE",reason:a}),alert("Approval granted successfully!"),window.location.hash="#/approvals"}catch(e){alert("Error: "+(e.message||"Approval failed"))}}),n&&n.addEventListener("click",async()=>{const a=prompt("Reason for returning (required):");if(!a){alert("A reason is required to return for revision.");return}try{await p.post(`/approvals/${t}/decide`,{decision:"RETURN",reason:a}),alert("Returned for revision."),window.location.hash="#/approvals"}catch(e){alert("Error: "+(e.message||"Action failed"))}}),r&&r.addEventListener("click",async()=>{const a=prompt("Reason for rejection (required):");if(!a){alert("A reason is required to reject.");return}try{await p.post(`/approvals/${t}/decide`,{decision:"REJECT",reason:a}),alert("Approval rejected."),window.location.hash="#/approvals"}catch(e){alert("Error: "+(e.message||"Action failed"))}})}async function tt(){try{const t=await p.get("/products");return Array.isArray(t)?t:[]}catch{return[]}}function et(t=[]){const s=t.length>0?t:[],n=s.filter(o=>o.is_active!==!1).length,r=s.filter(o=>o.is_active===!1).length,a=new Set(s.map(o=>o.category)),e=s.length===0?`
      <tr>
    ...
    `:s.map(o=>{const i=Number(o.base_price||0),l=Number(o.tax_rate||0),c=o.unit||"Each",d=o.is_active!==!1,u=d?"badge-success":"badge-error",m=d?"Active":"Archived";return`
  ...
  `}function st(){const t=document.getElementById("btn-new-product");t&&t.addEventListener("click",()=>{window.location.hash="#/product-detail/new"}),document.querySelectorAll(".product-row").forEach(s=>{s.addEventListener("click",()=>{const n=s.getAttribute("data-product-id");window.location.hash=`#/product-detail/${n}`})})}async function at(t){if(t==="new")return{product:null,variants:[]};try{const s=await p.get("/products"),n=Array.isArray(s)?s.find(a=>String(a.id)===String(t)):null;let r=[];if(n)try{r=await p.get(`/products/${t}/variants`)}catch{r=[]}return{product:n,variants:Array.isArray(r)?r:[]}}catch{return{product:null,variants:[]}}}function nt(t){const{product:s,variants:n}=t,r=!s,a=(s==null?void 0:s.name)||"",e=(s==null?void 0:s.category)||"",o=(s==null?void 0:s.base_price)||"",i=(s==null?void 0:s.unit)||"",l=(s==null?void 0:s.description)||"",c=(s==null?void 0:s.tax_rate)||"",d="",u={};(n||[]).forEach(x=>{u[x.attribute_name]||(u[x.attribute_name]={values:[],extraPrices:[]}),u[x.attribute_name].values.push(x.value),u[x.attribute_name].extraPrices.push(Number(x.extra_price||0))});const m=Object.keys(u).length>0?Object.entries(u).map(([x,f])=>{const h=f.values.join(", "),w=f.extraPrices.map(k=>k===0?"0":`+$${k}`),$=[...new Set(w)];return`
          <tr>
  ...
  `}function rt(t){const s=document.getElementById("pd-subscription"),n=document.getElementById("recurring-group");if(s&&n){const e=()=>{n.style.display=s.value==="yes"?"grid":"none"};s.addEventListener("change",e),e()}const r=document.getElementById("btn-add-variant");r&&t&&t!=="new"&&r.addEventListener("click",async()=>{var l,c,d,u,m;const e=(c=(l=document.getElementById("new-attr-name"))==null?void 0:l.value)==null?void 0:c.trim(),o=(u=(d=document.getElementById("new-attr-value"))==null?void 0:d.value)==null?void 0:u.trim(),i=parseFloat((m=document.getElementById("new-attr-price"))==null?void 0:m.value)||0;if(!e||!o){alert("Please fill in attribute name and value.");return}r.disabled=!0;try{await p.post(`/products/${t}/variants`,{attribute_name:e,value:o,extra_price:i}),window.location.hash=`#/product-detail/${t}`,window.dispatchEvent(new HashChangeEvent("hashchange"))}catch(x){alert(x.message||"Failed to add variant"),r.disabled=!1}});const a=document.getElementById("btn-save-product");a&&a.addEventListener("click",async()=>{var m,x,f,h,w,$,k,S,P,B;const e=(x=(m=document.getElementById("pd-name"))==null?void 0:m.value)==null?void 0:x.trim(),o=(h=(f=document.getElementById("pd-category"))==null?void 0:f.value)==null?void 0:h.trim(),i=parseFloat((w=document.getElementById("pd-price"))==null?void 0:w.value)||0,l=(k=($=document.getElementById("pd-unit"))==null?void 0:$.value)==null?void 0:k.trim(),c=parseFloat((S=document.getElementById("pd-tax"))==null?void 0:S.value)||0,d=((B=(P=document.getElementById("pd-description"))==null?void 0:P.value)==null?void 0:B.trim())||null;if(!e||!o||!l){alert("Please fill in product name, category, and unit.");return}const u={name:e,category:o,base_price:i,unit:l,tax_rate:c,description:d};a.disabled=!0,a.innerHTML='<span class="material-symbols-outlined text-lg">hourglass_top</span><span>Saving...</span>';try{if(!t||t==="new"){const C=await p.post("/products",u);window.location.hash=`#/product-detail/${C.id}`}else{try{await p.put(`/products/${t}`,u)}catch{}window.location.hash="#/products"}}catch(C){alert(C.message||"Failed to save product"),a.disabled=!1,a.innerHTML=`<span class="material-symbols-outlined text-lg">save</span><span>${t==="new"?"Create Product":"Save Changes"}</span>`}})}async function ot(){try{return await p.get("/discount-rules")}catch(t){return console.error("Failed to load discount rules",t),{tiers:[],categories:[],chains:[]}}}function it(t){const s=t.tiers||[],n=t.categories||[];t.chains;const r={Bronze:1,Silver:2,Gold:3};s.sort((o,i)=>(r[o.customer_tier]||99)-(r[i.customer_tier]||99));const a=s.map((o,i)=>`
    <tr>
  ...
  `}function lt(){const t=document.getElementById("btn-save-config");t&&t.addEventListener("click",async()=>{t.disabled=!0,t.textContent="Saving...";const s=document.querySelector(".page-container"),n=parseInt(s.getAttribute("data-tiers-count"))||0,r=parseInt(s.getAttribute("data-categories-count"))||0,a={tiers:[],categories:[],chains:[{min_score:1,required_level:"MANAGER_ONLY"}]};for(let e=0;e<n;e++)a.tiers.push({customer_tier:document.getElementById("tier-name-"+e).value,max_discount_percent:parseFloat(document.getElementById("tier-disc-"+e).value)});for(let e=0;e<r;e++)a.categories.push({category:document.getElementById("cat-name-"+e).value,max_discount_percent:parseFloat(document.getElementById("cat-disc-"+e).value)});try{await p.put("/discount-rules",a),alert("Configuration saved successfully!"),window.location.reload()}catch(e){alert("Error saving configuration: "+e.message),t.disabled=!1,t.textContent="Save configuration"}})}function ct(t=[]){const s=t.length>0?t:[{id:1,name:"Acme Corp Global ERP",email:"procurement@acmeww.com",tier:"Gold",created_at:"2025-01-15"},{id:2,name:"Starlight Pharma Logistics",email:"operations@starlightpharma.com",tier:"Silver",created_at:"2025-02-01"},{id:3,name:"Helios Solar Microgrid Infra",email:"infrastructure@heliosmicro.io",tier:"Gold",created_at:"2025-02-18"},{id:4,name:"Apex Financial Cloud Vault",email:"finops@apexvault.com",tier:"Bronze",created_at:"2025-03-02"}],n=s.map(r=>{let a="badge-primary";const e=(r.tier||"Bronze").toLowerCase();return e.includes("gold")?a="badge-warning":e.includes("silver")&&(a="badge-neutral"),`
      <tr class="table-row border-b border-surface-container-high/40 hover:bg-surface-container/40">
        ...
        <td class="py-3 px-4">
          <span class="badge ${a} text-[10px] font-bold">${r.tier||"Bronze"}</span>
        </td>
        ...
        <div class="card card-extruded flex items-center justify-between">
          <div>
            ...
            <div class="text-xl font-bold text-tertiary mt-1">
              ${s.filter(r=>(r.tier||"").toLowerCase().includes("gold")).length} Accounts
            </div>
          </div>
          <div class="icon-circle bg-tertiary-container/40">
  ...
  `}async function dt(){try{return await p.get("/customers")}catch{return[]}}function pt(){const t=document.getElementById("cust-search-input"),s=document.getElementById("customers-table");t&&s&&t.addEventListener("input",r=>{const a=r.target.value.toLowerCase();s.querySelectorAll("tbody tr").forEach(o=>{o.style.display=o.textContent.toLowerCase().includes(a)?"":"none"})});const n=document.getElementById("btn-add-customer");n&&n.addEventListener("click",()=>{E.show({title:"Add Enterprise Client Account",content:`
          <div class="space-y-3 text-xs">
            <div>
  ...
  `}async function mt(t){try{const s=await p.get("/quotations"),n=t?s.find(r=>r.id===parseInt(t,10)):s[0];return n?{quotation:await p.get(`/quotations/${n.id}`).catch(()=>n)}:{quotation:null}}catch{return{quotation:null}}}function xt(t){const s=(t==null?void 0:t.id)||8492,n=document.getElementById("btn-portal-accept");n&&n.addEventListener("click",async()=>{if(confirm("Confirm digital signature and ratify this commercial agreement?"))try{await p.put(`/quotations/${s}`,{status:"Confirmed"}),alert("Quotation digitally signed and ratified! Deal status updated to Confirmed."),window.location.reload()}catch(o){alert(o.message||"Signature failed")}});const r=document.getElementById("btn-send-portal-msg"),a=document.getElementById("portal-reply-text"),e=document.getElementById("portal-messages-list");r&&a&&e&&r.addEventListener("click",()=>{const o=a.value.trim();if(!o)return;const i=`
        <div class="p-3 rounded-2xl bg-surface-container-lowest border border-primary-container/60 shadow-sm ml-4">
          <div class="flex items-center justify-between mb-1">
            <span class="font-bold text-secondary">Procurement Lead (Acme Corp)</span>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="card card-extruded flex items-center justify-between">
          <div>
            ...
            <div class="text-xl font-bold font-mono text-primary mt-1">${r} of ${s.length} Paid</div>
          </div>
          <div class="icon-circle bg-surface-container-high/60">
  ...
  `}async function St(t){try{return await p.get(`/payments/invoices/${t}`)}catch(s){return console.warn("Could not fetch invoice detail:",s),null}}function Pt(t){if(!t)return;const s=document.getElementById("btn-download-pdf");s&&s.addEventListener("click",async()=>{try{const a=`DealFlow360_Invoice_${t.number||t.id}.pdf`;await p.downloadFile(`/payments/invoices/${t.id}/pdf`,a)}catch(a){alert("Error generating PDF: "+a.message)}});const n=document.getElementById("btn-send-reminder");n&&n.addEventListener("click",()=>{alert(`Dispatched automated billing reminder to ${t.customer_name||"Buyer"} AP desk.`)});const r=document.getElementById("btn-record-payment");r&&r.addEventListener("click",()=>{const a=r.getAttribute("data-amount");E.show({title:"Record Payment Signoff",content:`
    ...
    <section class="card card-extruded"><div class="flex items-center gap-2 mb-3"><span class="material-symbols-outlined text-primary">account_balance</span><h2 class="text-base font-bold">Remittance &amp; Contract Context</h2></div><div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs"><div><span class="detail-label">Customer entity</span><strong>${((o=t.customers)==null?void 0:o.join(", "))||"Unassigned account"}</strong></div><div><span class="detail-label">Payment terms</span><strong>Net 30 · Direct wire</strong></div><div><span class="detail-label">Ledger status</span><strong class="text-primary">Reconciled and monitored</strong></div></div></section></div>`}async function Rt(t){try{return await p.get(`/subscriptions/plans/${t}/billing-detail`)}catch(s){return console.warn("Could not fetch subscription billing detail:",s),null}}function jt(t){var s,n;(s=document.getElementById("btn-modify-subscription"))==null||s.addEventListener("click",()=>E.show({title:"Modify Subscription Terms",content:`<div class="space-y-3 text-xs"><label class="block font-bold">Plan name<input id="modify-plan-name" class="input-clay w-full mt-1" value="${t.name}" /></label><label class="block font-bold">Recurring price (INR)<input id="modify-plan-price" type="number" min="0.01" class="input-clay w-full mt-1" value="${t.price}" /></label><label class="block font-bold">Cadence<select id="modify-plan-cadence" class="input-clay w-full mt-1"><option value="monthly" ${t.cadence==="monthly"?"selected":""}>Monthly</option><option value="quarterly" ${t.cadence==="quarterly"?"selected":""}>Quarterly</option><option value="yearly" ${t.cadence==="yearly"?"selected":""}>Yearly</option></select></label></div>`,confirmText:"Save Terms",onConfirm:async()=>(await p.patch(`/subscriptions/plans/${t.id}`,{name:document.getElementById("modify-plan-name").value.trim(),price:Number(document.getElementById("modify-plan-price").value),cadence:document.getElementById("modify-plan-cadence").value}),window.location.hash=`#/subscriptions/${t.id}`,window.location.reload(),!0)})),(n=document.getElementById("btn-cancel-subscription"))==null||n.addEventListener("click",()=>{var a,e;const r=(e=(a=t==null?void 0:t.schedules)==null?void 0:a.find(o=>o.quotation_line_id))==null?void 0:e.quotation_line_id;E.show({title:"Cancel Subscription",content:'<p class="text-xs text-on-surface-variant">Future billing cycles will stop and this contract will be marked canceled.</p>',confirmText:"Cancel Contract",onConfirm:async()=>(r?await p.post(`/subscriptions/lines/${r}/cancel`,{}):await p.post(`/subscriptions/plans/${t.id}/cancel`,{}),window.location.hash="#/subscriptions",window.location.reload(),!0)})})}let g={period:"This Month",status:"All Statuses",product:"All Products"};async function R(t={}){try{const s=new URLSearchParams(t).toString();return{kpis:await p.get(`/reports/kpis?${s}`).catch(()=>null)}}catch{return{kpis:null}}}function Mt(t={}){const s=t.kpis||{quotes_created:0,avg_approval_time_hours:0,top_upsold_product:"None"},n=g.period.toLowerCase();return`
  ...
  `}function Ht(){var s,n,r,a,e;const t=async()=>{g={period:document.getElementById("filter-period").value,status:document.getElementById("filter-status").value,product:document.getElementById("filter-product").value};const o=await R(g);o&&o.kpis&&(document.getElementById("kpi-quotes").textContent=`${o.kpis.quotes_created} ${g.period.toLowerCase()}`,document.getElementById("kpi-time").textContent=`${o.kpis.avg_approval_time_hours} hours`,document.getElementById("kpi-product").textContent=`${o.kpis.top_upsold_product}`)};(s=document.getElementById("filter-period"))==null||s.addEventListener("change",t),(n=document.getElementById("filter-status"))==null||n.addEventListener("change",t),(r=document.getElementById("filter-product"))==null||r.addEventListener("change",t),(a=document.getElementById("filter-team"))==null||a.addEventListener("change",t),(e=document.getElementById("btn-export-pdf"))==null||e.addEventListener("click",()=>{var l,c,d;const o=new URLSearchParams({period:((l=document.getElementById("filter-period"))==null?void 0:l.value)||"This Month",status:((c=document.getElementById("filter-status"))==null?void 0:c.value)||"All Statuses",product:((d=document.getElementById("filter-product"))==null?void 0:d.value)||"All Products"}),i=window.location.port==="5173"?"":"http://localhost:8000";window.open(`${i}/api/v1/reports/export/pdf?${o.toString()}`,"_blank")})}function Ft(){return`
    <div class="page-container space-y-6">
      <!-- Top Banner -->
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
  ...
  `}function Ut(){const t=document.getElementById("btn-send-msg"),s=document.getElementById("msg-input"),n=document.getElementById("messages-list");if(t&&s&&n){const r=()=>{const a=s.value.trim();if(!a)return;const e=y.getUser()||{full_name:"You"},o=new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),i=document.createElement("div");i.className="flex flex-col items-start max-w-[80%]",i.innerHTML=`
        <div class="flex items-center gap-2 mb-1">
      ...
      `,n.appendChild(i),s.value="",n.scrollTop=n.scrollHeight};t.addEventListener("click",r),s.addEventListener("keydown",a=>{a.key==="Enter"&&r()})}}function Ot(){const t=y.getUser()||{full_name:"Alice Johnson",email:"salesrep@dealflow360.com",role:"SalesRep",phone:"+1 (555) 234-5678",address:"742 Evergreen Terrace, San Francisco, CA 94107",age:"28"},s=t.full_name?t.full_name.split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2):"US";return`
  ...
  `}function Qt(){const t=document.getElementById("profile-form"),s=document.getElementById("prof-name"),n=document.getElementById("prof-phone"),r=document.getElementById("prof-age"),a=document.getElementById("prof-address"),e=document.getElementById("profile-msg"),o=document.getElementById("btn-profile-logout");t&&t.addEventListener("submit",i=>{i.preventDefault();const l=s.value.trim();if(!l)return;const c=y.getUser()||{};c.full_name=l,c.phone=n?n.value.trim():c.phone,c.age=r?r.value.trim():c.age,c.address=a?a.value.trim():c.address,y.setUser(c),e&&(e.classList.remove("hidden"),setTimeout(()=>e.classList.add("hidden"),3500))}),o&&o.addEventListener("click",()=>{confirm("Are you sure you want to sign out of DealFlow360?")&&y.logout()})}class Gt{constructor(){this.appEl=document.getElementById("app"),window.addEventListener("hashchange",()=>this.handleRoute())}init(){!window.location.hash||window.location.hash==="#/"?window.location.hash=y.isAuthenticated()?"#/quotations":"#/login":this.handleRoute()}parseHash(){const s=window.location.hash.slice(1)||"/login",[n]=s.split("?"),a=(n.startsWith("/")?n.slice(1):n).split("/"),e=a[0]||"quotations",o=a[1]||null,i=new URLSearchParams(window.location.hash.split("?")[1]||"");return{route:e,param:o,query:i}}showLoading(){this.appEl.innerHTML=`
    ...
    `}async handleRoute(){const{route:s,param:n,query:r}=this.parseHash(),a=y.isAuthenticated();if(!a&&s!=="login"&&s!=="signup"&&s!=="portal"){window.location.hash="#/login";return}if(a&&(s==="login"||s==="signup")){window.location.hash="#/quotations";return}switch(window.scrollTo(0,0),s){case"login":{this.appEl.innerHTML=j(),M();break}case"signup":{this.appEl.innerHTML=H(),F();break}case"dashboard":{const e=y.getUser();if((e?e.selected_role||e.role:"")!=="Admin"){window.location.hash="#/quotations";break}this.showLoading();const i=await O();this.appEl.innerHTML=`
          ${b("dashboard")}
        ...
        `,v(),Q();break}case"quotations":{if(n){this.showLoading();const e=await D(n);this.appEl.innerHTML=`
          ...
          `,v(),T(n,e.products,e.customers)}else{this.showLoading();let e=!0,o="kanban";const i=async(l=e,c=o)=>{e=l,o=c;const d=await V(l);this.appEl.innerHTML=`
              ${b("quotations")}
              <main class="main-content">${G(d,l?"my":"all",c)}</main>
            `,v(),z(u=>i(u,o),u=>i(e,u))};await i(!0,"kanban")}break}case"quotation-detail":{this.showLoading();const e=n||r.get("id"),o=await D(e);this.appEl.innerHTML=`
          ${b("quotations")}
          <main class="main-content">${N(o)}</main>
        `,v(),T(e,o.products,o.customers);break}case"approvals":{this.showLoading();const e=await W();this.appEl.innerHTML=`
          ${b("approvals")}
          <main class="main-content">${Y(e)}</main>
        `,v(),J();break}case"approval-detail":{this.showLoading();const e=n||r.get("id"),o=await K(e);this.appEl.innerHTML=`
          ${b("approvals")}
          <main class="main-content">${X(o)}</main>
        `,v(),Z(e);break}case"products":{this.showLoading();const e=await tt();this.appEl.innerHTML=`
          ${b("products")}
          <main class="main-content">${et(e)}</main>
        `,v(),st();break}case"product-detail":{this.showLoading();const e=n||"new",o=await at(e);this.appEl.innerHTML=`
          ${b("products")}
