...
def create_product(data:ProductCreate,db:Session=Depends(get_db),user=Depends(require_role("Admin"))):
    ...
    item=Product(name=data.name,category=data.category,base_price=data.base_price,unit=data.unit,tax_rate=data.tax_rate,description=data.description); db.add(item);db.commit();db.refresh(item);return item
